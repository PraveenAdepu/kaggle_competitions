...
#    model7.add(Dense(num_dense/2, activation=act))
    ...
    merged_model.add(Merge([model5, model6, model_inp_reg,model_inp_city,model_inp_cat1,model_inp_cat2,model_inp_prm1,model_inp_prm2,model_inp_prm3,model_inp_sqnm,model_inp_usr,model_inp_itype,model7], mode='concat'))
...
def lstmnet(i):
    ...
    for j in range(1,nbags+1):
        ...
        model.fit([data_1_train, data_2_train,train_data_reg_train,train_data_city_train,train_data_cat1_train,train_data_cat2_train,train_data_prm1_train,train_data_prm2_train,train_data_prm3_train,train_data_sqnm_train,train_data_usr_train,train_data_itype_train,train_user_features_train], labels_train, 
                  validation_data=([data_1_val, data_2_val,train_data_reg_val,train_data_city_val,train_data_cat1_val,train_data_cat2_val,train_data_prm1_val,train_data_prm2_val,train_data_prm3_val,train_data_sqnm_val,train_data_usr_val,train_data_itype_val,train_user_features_val], labels_val), 
                  batch_size=batchsize, 
                  nb_epoch=epochs,
                  verbose=verboselog

                 )
        bag_cv   = model.predict([data_1_val, data_2_val,train_data_reg_val,train_data_city_val,train_data_cat1_val,train_data_cat2_val,train_data_prm1_val,train_data_prm2_val,train_data_prm3_val,train_data_sqnm_val,train_data_usr_val,train_data_itype_val,train_user_features_val], batch_size=batchsize, verbose=1)[:,0]
        pred_cv += model.predict([data_1_val, data_2_val,train_data_reg_val,train_data_city_val,train_data_cat1_val,train_data_cat2_val,train_data_prm1_val,train_data_prm2_val,train_data_prm3_val,train_data_sqnm_val,train_data_usr_val,train_data_itype_val,train_user_features_val], batch_size=batchsize, verbose=1)[:,0]
        
        pred_test += model.predict([test_data_1, test_data_2,test_data_reg,test_data_city,test_data_cat1,test_data_cat2,test_data_prm1,test_data_prm2,test_data_prm3,test_data_sqnm,test_data_usr,test_data_itype,test_user_features], batch_size=batchsize, verbose=1)[:,0]
    ...
    sub_valfile = inDir+'/submissions/Prav.nn04.fold' + str(i) + '.csv'    
    ...
    sub_file = inDir+'/submissions/Prav.nn04.fold' + str(i) + '-test' + '.csv'
...
#        model.load_weights('C:/Users/SriPrav/Documents/R/23Quora/nn2_question_pairs_weights.h5')
    ...
    pred_test = pred_test[['item_id','deal_probability']]
    pred_test['deal_probability'] = pred_test['deal_probability'].clip(0.0, 1.0)
    sub_file = inDir+'/submissions/Prav.nn04.full' + '.csv'
    pred_test.to_csv(sub_file, index=False)
