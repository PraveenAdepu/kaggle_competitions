...
MODEL_WEIGHTS_FILE = inDir + '/Prav_01_InceptionV3.h5'
...
def batch_generator_train(images_build ,batch_size, shuffle):
    ...
    while True:
        ...
        X_batch = InceptionV3_preprocess_input(X_batch)
        counter += 1
...
def model_InceptionV3(num_classes):
    base_model = InceptionV3(include_top=False, weights='imagenet',input_shape=(CHANNELS, ROWS, COLUMNS))
...
model = model_InceptionV3(num_classes=num_classes) 
...
def InceptionV3_preprocess_input_test(x):
    x=x.transpose((0,3,1,2))
    x= x.astype('float32')
    x /= 255.
    x -= 0.5
...
def img2feat(im):
    ...
    x= InceptionV3_preprocess_input_test(x)
    return x
...
submission.to_csv(inDir+'/submissions/Prav_01_InceptionV3.csv')
