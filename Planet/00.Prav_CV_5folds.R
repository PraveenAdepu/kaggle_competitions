...
train  <- read_csv("./input/train_v2.csv") 

names(train)

head(train$image_name,5)
# Random shuffle dataset row wise
train <- train[sample(nrow(train)),]
head(train$image_name,5)

CVColumns <- c("image_name")
train <- as.data.frame(train$image_name)

names(train)[1] <- "image_name"


folds <- createFolds(train$image_name, k = 10)
...
names(trainingFold01)[1] <- "image_name"
names(trainingFold02)[1] <- "image_name"
names(trainingFold03)[1] <- "image_name"
names(trainingFold04)[1] <- "image_name"
names(trainingFold05)[1] <- "image_name"
names(trainingFold06)[1] <- "image_name"
names(trainingFold07)[1] <- "image_name"
names(trainingFold08)[1] <- "image_name"
names(trainingFold09)[1] <- "image_name"
names(trainingFold10)[1] <- "image_name"
...
trainingFolds <- trainingFolds[with(trainingFolds, order(image_name)), ]
