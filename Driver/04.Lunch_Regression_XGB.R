...
validation.pred.columns <- c("ID","Script Directions","CVindices","Lunch")
  ...
  
{
  ...
  dtrain <-xgb.DMatrix(data=data.matrix(X_build[, feature.names]),label=X_build$Lunch)
  dval   <-xgb.DMatrix(data=data.matrix(X_val[, feature.names])  ,label=X_val$Lunch)
  ...
  names(X_val_pred) <- "LunchProbability"
  if(i == 1)
...
sum(OOF_Lunch$LunchProbability)
sum(OOF_Lunch$Lunch)
# Full training
...
# dtrain<-xgb.DMatrix(data=data.matrix(trainingSet[, feature.names]),label=trainingSet$Lunch)
...
# testfull_predictions  <- data.frame(id=testingSet$id, LunchProbability = round(predfull_test))
