...
def train_xgboost_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,xgbParameters, num_rounds):
    ...
    for j in range(1,nbags+1):
        ...
        xgbParameters[7] = ('seed', current_seed + j)         
...
def nn_model(size):
    ...
    model.add(Dense(250, input_dim = size, init = 'he_normal'))
    model.add(PReLU())
    ...
    model.add(Dense(60, init = 'he_normal'))
    model.add(PReLU())
    model.add(BatchNormalization())
    model.add(Dropout(0.2))
...
def nn_model(size): # create model
	model = Sequential()
	model.add(Dense(16, input_dim=size, init='normal', activation='relu'))
	model.add(Dropout(0.2))
...
# lgb regression ############################################################################################################################
#############################################################################################################################################
def train_nn_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName, nn_epoch):
    ...
    for j in range(1,nbags+1):
        ...
        model = nn_model(len(feature_names))
        model.fit( X_build[feature_names].values, X_build['DiabetesDispense'].values, batch_size=120, nb_epoch=nn_epoch, validation_data=(X_valid[feature_names].values,  X_valid['DiabetesDispense'].values), verbose=2)
        
        bag_cv= model.predict(X_valid[feature_names].values #,num_iteration=model.best_iteration
                             )[:,0] #.reshape(-1,1)
        pred_test += model.predict(testingSet[feature_names].values)[:,0]
        pred_cv += bag_cv
        bag_score = roc_auc_score(X_valid['DiabetesDispense'], bag_cv)
        print('bag ', j, '- AUC:', bag_score)
    pred_cv /= nbags
...
def fulltrain_nn_regression(trainingSet, testingSet,feature_names,nbags,ModelName, nn_epoch):

    predfull_test = np.zeros(testingSet.shape[0]) 
    for j in range(1,nbags+1):
        print('bag ', j , ' Processing')
        fullmodel = nn_model(len(feature_names))
        fullmodel.fit( trainingSet[feature_names].values, trainingSet['DiabetesDispense'].values, batch_size=120, nb_epoch=nn_epoch, verbose=2)
    
        predfull_test += fullmodel.predict(testingSet[feature_names].values)[:,0]
    predfull_test/= nbags
    predfull_test = pd.DataFrame(predfull_test)
    predfull_test.columns = ["Diabetes"]
    predfull_test["Patient_ID"] = testingSet.Patient_ID.values
...
def train_regression(RegressionModelName,trainingSet, testingSet,feature_names,i,nbags,ModelName,current_seed):
    if RegressionModelName in ["rf","ada","gbdt","et","lr","lsvc","knn"]:    
        X_build = trainingSet[trainingSet['CVindices'] != i]
        X_valid   = trainingSet[trainingSet['CVindices'] == i]      
        ...
        for j in range(1,nbags+1):
            ...
            if RegressionModelName == "rf":
                RegressionModel = RandomForestRegressor(n_estimators=600, max_depth=10, n_jobs=20, random_state=bag_seed, max_features="auto",verbose=1)
            if RegressionModelName == "ada":
                RegressionModel = AdaBoostRegressor(n_estimators=500, random_state=bag_seed, learning_rate=0.01)
            if RegressionModelName == "gbdt":
                RegressionModel = GradientBoostingRegressor(n_estimators=500,learning_rate=0.04,  subsample=0.8, random_state=bag_seed,max_depth=5,verbose=1)
            if RegressionModelName == "et":
                RegressionModel = ExtraTreesRegressor(n_estimators=600, max_depth=10, max_features="auto", n_jobs=20, random_state=bag_seed,verbose=1)
    ...
    if RegressionModelName in ["rf","ada","gbdt","et","lr","lsvc","knn"]:    
        ...
        for j in range(1,nbags+1):
            ...
            if RegressionModelName == "rf":
                RegressionModel = RandomForestRegressor(n_estimators=600, max_depth=10, n_jobs=20, random_state=bag_seed, max_features="auto",verbose=1)
            if RegressionModelName == "ada":
                RegressionModel = AdaBoostRegressor(n_estimators=550, random_state=bag_seed, learning_rate=0.01)
            if RegressionModelName == "gbdt":
                RegressionModel = GradientBoostingRegressor(n_estimators=550,learning_rate=0.04,  subsample=0.8, random_state=bag_seed,max_depth=5,verbose=1)
            if RegressionModelName == "et":
                RegressionModel = ExtraTreesRegressor(n_estimators=660, max_depth=10, max_features="auto", n_jobs=20, random_state=bag_seed,verbose=1)
            if RegressionModelName == "lr":
                RegressionModel = LinearRegression(n_jobs=20) 
            if RegressionModelName == "knn":
                RegressionModel = KNeighborsRegressor(n_neighbors=2,n_jobs=20)
