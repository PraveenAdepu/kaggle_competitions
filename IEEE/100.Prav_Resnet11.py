...
images_train1 = images_train1[images_train1['image_patch_rank']<=10]

images_train2 = pd.read_csv(inDir + '/input/Prav_5folds_CVindices_v51.csv')
images_train2 = images_train2[images_train2['image_patch_rank']<=2]

images_train3 = pd.read_csv(inDir + '/input/Prav_5folds_AugmentationCVindices_v2.csv')
images_train3["image_name_patch"] = "unknown"
images_train3["extension"] = "0.jpg"
images_train3["image_patch_number"] = 0
images_train3["image_patch_rank"] = 1
del images_train3["image_name1"]


images_train = pd.concat([images_train1, images_train2, images_train3])
...
def get_im_cv2(path,use_manipulation=True):
    img = cv2.imread(path)
...
#        print(fl)        
        img = get_im_cv2(fl,use_manipulation=True)        
        X_train.append(img)
...
def load_test_frombatch(images_batch):
    X_test = []
    for fl in images_batch.image_path.values:
        img = get_im_cv2(fl,use_manipulation=False)        
        X_test.append(img)
    return X_test
...
reduce_lr = ReduceLROnPlateau(monitor='val_acc', factor=0.5, patience=1, min_lr=1e-9, epsilon = 0.00001, verbose=1, mode='max')
nb_epoch = 5
learning_rate = 1e-3
...
model.fit_generator( generator=batch_generator_X_build(images_build,X_build, y_build, batch_size, shuffle=True),
                         ...
                         steps_per_epoch = math.ceil((len(images_build)/2) / batch_size), #int(len(build_index)/float(batch_size)),
                         nb_epoch = nb_epoch, 
