...
require(e1071)
require(nnet)
nnetCtrl <- trainControl(method="cv",	        # use repeated 10fold cross validation
                        #repeats=5,
...
nnetGrid <- expand.grid(decay = c(0.1), size = c(8)
...
system.time(nnet.tune <- train(x=Fulltrain[,featureNames],y=Fulltrain$fault_severity,
                                 method = "nnet",
                                 metric = "logLoss",
                                 trControl = nnetCtrl,
                                 tuneGrid=nnetGrid,
...
# Training error rate
confusionMatrix(predict(nnet.tune, Fulltrain[,featureNames]), Fulltrain$fault_severity)
# Accuracy : 0.6931
#Accuracy : 0.8859
#Accuracy : 0.8832
# predictedClasses <- predict(svm.c, Fulltest[,TestfeatureNames] )
predictedProbs   <- predict(nnet.tune, Fulltest[,TestfeatureNames], type = "prob")
