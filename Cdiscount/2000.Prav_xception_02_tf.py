...
model.compile(optimizer=optim, loss='categorical_crossentropy', metrics=[top_k_categorical_accuracy, 'accuracy'])
model.summary()
...
model.fit_generator( generator=batch_generator_train(images_build, batch_size, shuffle=True),
                             ...
                             steps_per_epoch = math.ceil(len(build_index) / batch_size), #int(len(build_index)/float(batch_size)),math.ceil(2000000 / batch_size), #
                             nb_epoch = nb_epoch, 
                             callbacks = callbacks,
                             validation_data=batch_generator_valid(images_valid, batch_size, shuffle=True), 
                             #nb_val_samples=len(valid_index), 
                             validation_steps = math.ceil(len(valid_index) / batch_size), #int(len(valid_index)/float(batch_size)),math.ceil(10000 / batch_size), #
                             max_q_size=10,
                             verbose = VERBOSEFLAG 
                  )
...
submission.to_csv(inDir+'/submissions/Prav_xception_02.csv')
