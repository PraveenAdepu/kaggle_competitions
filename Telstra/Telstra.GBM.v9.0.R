...
events       <- sqldf("SELECT id, location, fault_severity, event_type , 1 as [AvgEventVolume],MAX(volume) as [MaxEventVolume],   MIN(volume) as [MinEventVolume] FROM df_all_combined GROUP BY 1,2,3,4")

resources    <- sqldf("SELECT id, location, fault_severity, resource_type , 1 as [AvgResourceVolume] , MAX(volume) as [MaxResourceVolume], MIN(volume) as [MinResourceVolume]FROM df_all_combined GROUP BY 1,2,3,4")

Severities   <- sqldf("SELECT id, location, fault_severity, severity_type , 1 as [AvgSeverityVolume] , MAX(volume) as [MaxSeverityVolume] , MIN(volume) as [MinSeverityVolume]FROM df_all_combined GROUP BY 1,2,3,4")
...
gbmGrid <- expand.grid(interaction.depth = c(30), # seq(1,8,by=2), # look at tree depths from 1 to 7
...
system.time(gbm.tune <- train(x=Fulltrain[,featureNames],y=Fulltrain$fault_severity,
                              method = "gbm",
                              metric = "mlogloss",
                              trControl = gbmCtrl,
                              tuneGrid=gbmGrid,
...
# Training error rate
confusionMatrix(predict(gbm.tune, Fulltrain[,featureNames]), Fulltrain$fault_severity)
# Accuracy : 0.6931
#Accuracy : 0.8859
#Accuracy : 0.8832
# predictedClasses <- predict(svm.c, Fulltest[,TestfeatureNames] )
predictedProbs   <- predict(gbm.tune, Fulltest[,TestfeatureNames], type = "prob")
...
write.csv(prediction, "submission35.csv", quote=FALSE, row.names = FALSE)
