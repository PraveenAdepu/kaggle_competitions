...
learning_rate = 1e-3
VERBOSEFLAG = 2
MODEL_WEIGHTS_FILE = inDir + '/Unet_CCN03_weights.h5'
...
#    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=[mean_iou]) #, metrics=[mean_iou]) # The mean_iou metrics seens to leak train and test values...
    ...
    callbacks = [
                ...
                ReduceLROnPlateau(patience=3, verbose=VERBOSEFLAG,factor=0.1,min_lr=learning_rate),
...
#            ]
    ...
    results = model.fit({'img': X_build, 'feat': X_build_feat}, y_build, batch_size=batch_size, epochs=nb_epoch, callbacks=callbacks, verbose=2,
                        validation_data=({'img': X_valid, 'feat': X_valid_feat}, y_valid))
    model.load_weights(MODEL_WEIGHTS_FILE)
    model.evaluate({'img': X_valid, 'feat': X_valid_feat}, y_valid, verbose=1)
    
    preds_val = model.predict({'img': X_valid, 'feat': X_valid_feat}, verbose=1)
    preds_test = model.predict({'img': X_test_current, 'feat': X_feat_test_current}, verbose=1)
...
#                                           (sizes_test[i][0], sizes_test[i][1]), 
    ...
    pred_test_pkl = 'preds_test'+str(fileNo)+'.pkl'
    y_val_pkl = 'y_val'+str(fileNo)+'.pkl'
    pred_val_pkl = 'preds_val'+str(fileNo)+'.pkl'
