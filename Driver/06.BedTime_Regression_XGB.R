...
validation.pred.columns <- c("ID","Script Directions","CVindices","BedTime")
  ...
  
{
  ...
  dtrain <-xgb.DMatrix(data=data.matrix(X_build[, feature.names]),label=X_build$BedTime)
  dval   <-xgb.DMatrix(data=data.matrix(X_val[, feature.names])  ,label=X_val$BedTime)
  ...
  names(X_val_pred) <- "BedTimeProbability"
  if(i == 1)
...
OOF_BedTime <- OOF_preds
rm(OOF_preds)
head(OOF_BedTime)

sum(OOF_BedTime$BedTimeProbability)
sum(OOF_BedTime$BedTime)
# Full training
...
# dtrain<-xgb.DMatrix(data=data.matrix(trainingSet[, feature.names]),label=trainingSet$BedTime)
...
# testfull_predictions  <- data.frame(id=testingSet$id, BedTimeProbability = round(predfull_test))
