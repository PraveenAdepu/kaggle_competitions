...
from keras.layers import Dense, Embedding, Input, Flatten, Merge, merge
...
trainingSet['count_sent']=trainingSet["comment_text"].apply(lambda x: len(re.findall("\n",str(x)))+1)
#Word count in each comment:
trainingSet['count_word']=trainingSet["comment_text"].apply(lambda x: len(str(x).split()))
#Unique word count
trainingSet['count_unique_word']=trainingSet["comment_text"].apply(lambda x: len(set(str(x).split())))
#Letter count
trainingSet['count_letters']=trainingSet["comment_text"].apply(lambda x: len(str(x)))
#punctuation count
trainingSet["count_punctuations"] =trainingSet["comment_text"].apply(lambda x: len([c for c in str(x) if c in string.punctuation]))
#upper case words count
trainingSet["count_words_upper"] = trainingSet["comment_text"].apply(lambda x: len([w for w in str(x).split() if w.isupper()]))
#title case words count
trainingSet["count_words_title"] = trainingSet["comment_text"].apply(lambda x: len([w for w in str(x).split() if w.istitle()]))
#Number of stopwords
#trainingSet["count_stopwords"] = trainingSet["comment_text"].apply(lambda x: len([w for w in str(x).lower().split() if w in eng_stopwords]))
#Average length of the words
trainingSet["mean_word_len"] = trainingSet["comment_text"].apply(lambda x: np.mean([len(w) for w in str(x).split()]))
...
feature_columns = ['count_sent','count_word','count_unique_word','count_letters',"count_punctuations","count_words_upper","count_words_title","mean_word_len"]

trainingSet[feature_columns].fillna(0, inplace=True)
test[feature_columns].fillna(0, inplace=True)
...
trainingSet, test = normalise_contineous_features(trainingSet, test ,feature_columns)
...
def CNN_model():
    ...
    inp = Input(shape=(MAX_SEQUENCE_LENGTH, ))
    x = Embedding(nb_words, EMBEDDING_DIM, weights=[embedding_matrix], input_length=MAX_SEQUENCE_LENGTH,trainable=False)(inp)
    
    conv1 = Conv1D(128, 1, padding='same',activation='relu')
    conv2 = Conv1D(128, 2, padding='same',activation='relu')
    conv3 = Conv1D(128, 3, padding='same',activation='relu')
    conv4 = Conv1D(128, 4, padding='same',activation='relu')
    conv5 = Conv1D(32,  5, padding='same',activation='relu')
    conv6 = Conv1D(32,  6, padding='same',activation='relu')
    ...
    merged = merge([glob1a,glob2a,glob3a,glob4a,glob5a,glob6a], mode='concat', concat_axis=1)
...
def CNN_model2():
    ...
    # Add feature engineering features
    distance_input = Input(shape=(8,))
    distance_dense = BatchNormalization()(distance_input)
    distance_dense = Dense(32, activation="relu")(distance_dense)
       
    merged = merge([glob1a,glob2a,glob3a,glob4a,glob5a,glob6a,distance_dense], mode='concat', concat_axis=1)
    ...
    x = Dense(64, activation="relu")(x)
    x = Dense(6, activation="sigmoid")(x)
    model = Model(inputs=[inp,distance_input], outputs=x)    

    return model

MODEL_WEIGHTS_FILE = inDir + '/106_Prav_CNN.h5'
...
ModelName = '106_Prav_CNN'

X_test_features = test[feature_columns].values
...
def lstmnet(i):
    ...
    X_build_features = X_build[feature_columns].values
    X_valid_features = X_val[feature_columns].values
    ...
    for j in range(1,nbags+1):
        ...
        model.fit([X_build_data,X_build_features], y_build, 
                  validation_data=([X_valid_data,X_valid_features], y_valid), 
        ...
        bag_cv  += model.predict([X_valid_data,X_valid_features], batch_size=batch_size, verbose=VERBOSEFLAG)        
        pred_cv += model.predict([X_valid_data,X_valid_features], batch_size=batch_size, verbose=VERBOSEFLAG)        
        pred_test += model.predict([X_test,X_test_features], batch_size=batch_size, verbose=VERBOSEFLAG)
