...
train_sub <- train[date >= as.Date("2016-07-26"), ]
...
fcst_matrix <- foreach(i=1:nrow(train_ts),.combine=rbind, .packages=c("forecast")) %dopar% { 
  fcst_matrix <- expm1(forecast(ets(train_ts[i,-1]),h=fcst_intv)$mean)
}
...
write.csv(submission, "./submissions/ets_validation_2.csv", row.names =  FALSE, quote = FALSE)
