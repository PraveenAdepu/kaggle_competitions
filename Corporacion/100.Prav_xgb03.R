...
max(train_sub$date)
train_dates <- seq(as.Date('2017-01-01'),as.Date('2017-08-10'),by = 1)
train_dates <- as.data.frame(train_dates)
names(train_dates) <- "date"

train_stores <- train_sub %>% distinct(store_nbr)
train_items  <- train_sub %>% distinct(item_nbr)


train_store_items <- merge(train_stores,train_items,all=TRUE)

train_store_items_dates <- merge(train_store_items,train_dates,all=TRUE) # 47,975,976
...
X_build <- trainingSet[date < "2017-7-26"]
X_val   <- trainingSet[date >= "2017-7-26"]

# saveRDS(trainingSet, "./input/trainingSet.rds")
# saveRDS(testingSet, "./input/testingSet.rds")
