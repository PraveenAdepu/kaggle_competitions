...
#                                 )
#
#def batch_generator_X_build_augmentation(images_build,X_build, y_build ,batch_size, shuffle,data_gen_args=data_gen_args_train):
#    #chenglong code for fiting from generator (https://www.kaggle.com/c/talkingdata-mobile-user-demographics/forums/t/22567/neural-network-for-sparse-matrices)
...
#        
#        gen_X_batch = datagen.flow(X_batch,y_batch, batch_size=batch_size,shuffle=False)
#        while True:
#            X1i = gen_X_batch.next()            
...
#def CustomeNet(num_classes):
#    model = Sequential()
#    model.add(Conv2D(32, 3, padding="same", activation="relu", input_shape=(ROWS, COLUMNS,CHANNELS)))
...
#model.summary()
#   
#model.fit_generator( generator=batch_generator_train(images_build, batch_size, shuffle=True),
...
#                             nb_epoch = nb_epoch, 
#                             callbacks = callbacks,
#                             validation_data=batch_generator_valid(images_valid, batch_size, shuffle=True), 
...
#top1_acc = lambda x, y: top_k_categorical_accuracy(x, y, k=1)
 
ModelName= 'CustomNet_01'
i=1
...
#        model.fit_generator( generator=batch_generator_X_train(images_build, batch_size, shuffle=True),
#                                     #samples_per_epoch = len(build_index), 
#                                     steps_per_epoch = math.ceil(len(images_build) / batch_size), #int(len(build_index)/float(batch_size)),
#                                     nb_epoch = nb_epoch, 
#                                     callbacks = callbacks,
...
#        
    ...
    pred_cv = pred_cv[["fname",'HTC-1-M7', 'iPhone-4s', 'iPhone-6', 'LG-Nexus-5x',
    ...
    pred_test = pred_test[["fname",'HTC-1-M7', 'iPhone-4s', 'iPhone-6', 'LG-Nexus-5x',
       'Motorola-Droid-Maxx', 'Motorola-Nexus-6', 'Motorola-X',
       'Samsung-Galaxy-Note3', 'Samsung-Galaxy-S4', 'Sony-NEX-7']]
