...
head(X_build$cust_14)
X_build$cust_35 <- as.integer(str_sub(X_build$customer_id,3,5))
X_val$cust_35   <- as.integer(str_sub(X_val$customer_id,3,5))

X_build$cust_14 <- as.integer(str_sub(X_build$customer_id,1,4))
X_val$cust_14   <- as.integer(str_sub(X_val$customer_id,1,4))

sapply(X_build[, feature.names], class)
feature.names     <- names(X_build[,-which(names(X_build) %in% c("customer_id", "date" ,"target","id","f_19","f_29","customer_visit_date_lag1"
