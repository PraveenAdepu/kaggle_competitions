# -*- coding: utf-8 -*-
"""
Created on Sun Oct 29 00:15:29 2017
...
#Load data
train, train_bands = read_jason(file='train.json', loc=inDir + '/input/')
test, test_bands = read_jason(file='test.json', loc=inDir + '/input/')
X_train = []
for i in range(len(train)):
    img_features = image_to_stats(train_bands[i])
    X_train.append(img_features)
X_train = np.concatenate([X_train, train['inc_angle'].values[:, np.newaxis]], axis=-1)
...
for i in range(len(test)):
    img_features = image_to_stats(test_bands[i])
    X_test.append(img_features)
X_test = np.concatenate([X_test, test['inc_angle'].values[:, np.newaxis]], axis=-1)

trainfoldSource = pd.read_csv(inDir+'/input/Prav_4folds_CVindices.csv')
train = pd.merge(train, trainfoldSource, how='left',on="id")

train.groupby(['CVindices'])[['is_iceberg']].sum()
...
if __name__ == '__main__':
    for i in range(1, folds+1):
        train_xgboost_regression_usingIndex(train, test,X_train, y_train, X_test,i,nbags,xgbModelName,xgbParameters, xgb_num_rounds,current_seed,verboseeval)
