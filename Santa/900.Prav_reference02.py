# -*- coding: utf-8 -*-
"""
Created on Sun Dec 17 08:23:19 2017
...
inDir = 'C:\\Users\\SriPrav\\Documents\\R\\41Santa'
...
CHILD_PREF = pd.read_csv(inDir+'/input/child_wishlist.csv', header=None).drop(0, 1).values
GIFT_PREF = pd.read_csv(inDir+'/input/gift_goodkids.csv', header=None).drop(0, 1).values
...
BLOCK_SIZE = 400
file_anh = 0.937450
#INITIAL_SUBMISSION = inDir+'/input/cpp_sub.csv'
#INITIAL_SUBMISSION = inDir+'/input/heuristicSub.csv'
for file in range(54):
    INITIAL_SUBMISSION = inDir+('/submission_%i.csv'% int(file_anh * 10 ** 6))
    print(INITIAL_SUBMISSION)
    #INITIAL_SUBMISSION = inDir+'/input/submission_937438.csv'
