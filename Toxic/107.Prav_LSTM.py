# -*- coding: utf-8 -*-
"""
Created on Fri Dec 29 12:36:25 2017
...
#            stop_words = 'english')
##convert it to str or Unicode('U'), else TF_IDF throws nan error
#merge=pd.concat([train.iloc[:,0:2],test.iloc[:,0:2]])
#
#corpus=merge.comment_text.astype('U')
#tfv.fit(corpus)
#xtrain_tfv =  tfv.transform(trainingSet.comment_text.astype('U')) 
#xvalid_tfv = tfv.transform(test.comment_text.astype('U'))
#
#xtrain_tfv.shape
#xvalid_tfv.shape
...
def get_lstm_attention2():
    ...
    merged = merge([x,distance_dense], mode='concat', concat_axis=1)
    ...
    ## train the model
    ########################################
    model = Model(inputs=[comment_input,distance_input], \
            outputs=preds)
    return model

MODEL_WEIGHTS_FILE = inDir + '/107_Prav_LSTM.h5'
