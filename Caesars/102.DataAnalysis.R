...
test <- all.source %>% filter(customer_id =="144105526079")
...
summary(all.source$f_13)

generic.data.visualisations("continuous.hist", all.source,"target")
generic.data.visualisations("point", all.source, "f_13","target")
...
for (f in char.feature.names) {
  if (class(all.source[[f]])=="character") {
    cat("char feature : ", f ,"\n")
    levels <- unique(c(all.source[[f]]))
    all.source[[f]] <- as.integer(factor(all.source[[f]], levels=levels))
...
# all.source$customer_f3_lag1_diff  <-ifelse(is.na(all.source$customer_f3_lag1) , NA, all.source$f_3 - all.source$customer_f3_lag1   )
# all.source$customer_f4_lag1_diff  <-ifelse(is.na(all.source$customer_f4_lag1) , NA, all.source$f_4 - all.source$customer_f4_lag1)
# all.source$customer_f6_lag1_diff  <-ifelse(is.na(all.source$customer_f6_lag1) , NA, all.source$f_6 - all.source$customer_f6_lag1)
# all.source$customer_f8_lag1_diff  <-ifelse(is.na(all.source$customer_f8_lag1) , NA, all.source$f_8 - all.source$customer_f8_lag1)
# all.source$customer_f10_lag1_diff <-ifelse(is.na(all.source$customer_f10_lag1), NA, all.source$f_10 - all.source$customer_f10_lag1)
# all.source$customer_f11_lag1_diff <-ifelse(is.na(all.source$customer_f11_lag1), NA, all.source$f_11 - all.source$customer_f11_lag1)
# all.source$customer_f12_lag1_diff <-ifelse(is.na(all.source$customer_f12_lag1), NA, all.source$f_12 - all.source$customer_f12_lag1)
all.source$customer_f13_lag1_diff <-ifelse(is.na(all.source$customer_f13_lag1), NA, all.source$f_13 - all.source$customer_f13_lag1)
# all.source$customer_f14_lag1_diff <-ifelse(is.na(all.source$customer_f14_lag1), NA, all.source$f_14 - all.source$customer_f14_lag1)
all.source$customer_f15_lag1_diff <-ifelse(is.na(all.source$customer_f15_lag1), NA, all.source$f_15 - all.source$customer_f15_lag1)
# all.source$customer_f17_lag1_diff <-ifelse(is.na(all.source$customer_f17_lag1), NA, all.source$f_17 - all.source$customer_f17_lag1)
# all.source$customer_f18_lag1_diff <-ifelse(is.na(all.source$customer_f18_lag1), NA, all.source$f_18 - all.source$customer_f18_lag1)
# all.source$customer_f21_lag1_diff <-ifelse(is.na(all.source$customer_f21_lag1), NA, all.source$f_21 - all.source$customer_f21_lag1)
# all.source$customer_f22_lag1_diff <-ifelse(is.na(all.source$customer_f22_lag1), NA, all.source$f_22 - all.source$customer_f22_lag1)
# all.source$customer_f25_lag1_diff <-ifelse(is.na(all.source$customer_f25_lag1), NA, all.source$f_25 - all.source$customer_f25_lag1)
# all.source$customer_f26_lag1_diff <-ifelse(is.na(all.source$customer_f26_lag1), NA, all.source$f_26 - all.source$customer_f26_lag1)
# all.source$customer_f27_lag1_diff <-ifelse(is.na(all.source$customer_f27_lag1), NA, all.source$f_27 - all.source$customer_f27_lag1)
# all.source$customer_f28_lag1_diff <-ifelse(is.na(all.source$customer_f28_lag1), NA, all.source$f_28 - all.source$customer_f28_lag1)
all.source$customer_f30_lag1_diff <-ifelse(is.na(all.source$customer_f30_lag1), NA, all.source$f_30 - all.source$customer_f30_lag1)
# all.source$customer_f32_lag1_diff <-ifelse(is.na(all.source$customer_f32_lag1), NA, all.source$f_32 - all.source$customer_f32_lag1)
# all.source$customer_f34_lag1_diff <-ifelse(is.na(all.source$customer_f34_lag1), NA, all.source$f_34 - all.source$customer_f34_lag1)
# all.source$customer_f35_lag1_diff <-ifelse(is.na(all.source$customer_f35_lag1), NA, all.source$f_35 - all.source$customer_f35_lag1)
# all.source$customer_f36_lag1_diff <-ifelse(is.na(all.source$customer_f36_lag1), NA, all.source$f_36 - all.source$customer_f36_lag1)
# all.source$customer_f37_lag1_diff <-ifelse(is.na(all.source$customer_f37_lag1), NA, all.source$f_37 - all.source$customer_f37_lag1)
# all.source$customer_f38_lag1_diff <-ifelse(is.na(all.source$customer_f38_lag1), NA, all.source$f_38 - all.source$customer_f38_lag1)
# all.source$customer_f39_lag1_diff <-ifelse(is.na(all.source$customer_f39_lag1), NA, all.source$f_39 - all.source$customer_f39_lag1)
# all.source$customer_f40_lag1_diff <-ifelse(is.na(all.source$customer_f40_lag1), NA, all.source$f_40 - all.source$customer_f40_lag1)
# all.source$customer_f41_lag1_diff <-ifelse(is.na(all.source$customer_f41_lag1), NA, all.source$f_41 - all.source$customer_f41_lag1)

# all.source$customer_f3_lag1_div  <-ifelse(is.na(all.source$customer_f3_lag1) , NA, all.source$f_3 / all.source$customer_f3_lag1   )
# all.source$customer_f4_lag1_div  <-ifelse(is.na(all.source$customer_f4_lag1) , NA, all.source$f_4 / all.source$customer_f4_lag1)
# all.source$customer_f6_lag1_div  <-ifelse(is.na(all.source$customer_f6_lag1) , NA, all.source$f_6 / all.source$customer_f6_lag1)
# all.source$customer_f8_lag1_div  <-ifelse(is.na(all.source$customer_f8_lag1) , NA, all.source$f_8 / all.source$customer_f8_lag1)
# all.source$customer_f10_lag1_div <-ifelse(is.na(all.source$customer_f10_lag1), NA, all.source$f_10 / all.source$customer_f10_lag1)
# all.source$customer_f11_lag1_div <-ifelse(is.na(all.source$customer_f11_lag1), NA, all.source$f_11 / all.source$customer_f11_lag1)
# all.source$customer_f12_lag1_div <-ifelse(is.na(all.source$customer_f12_lag1), NA, all.source$f_12 / all.source$customer_f12_lag1)
all.source$customer_f13_lag1_div <-ifelse(is.na(all.source$customer_f13_lag1), NA, all.source$f_13 / all.source$customer_f13_lag1)
# all.source$customer_f14_lag1_div <-ifelse(is.na(all.source$customer_f14_lag1), NA, all.source$f_14 / all.source$customer_f14_lag1)
all.source$customer_f15_lag1_div <-ifelse(is.na(all.source$customer_f15_lag1), NA, all.source$f_15 / all.source$customer_f15_lag1)
# all.source$customer_f17_lag1_div <-ifelse(is.na(all.source$customer_f17_lag1), NA, all.source$f_17 / all.source$customer_f17_lag1)
# all.source$customer_f18_lag1_div <-ifelse(is.na(all.source$customer_f18_lag1), NA, all.source$f_18 / all.source$customer_f18_lag1)
# all.source$customer_f21_lag1_div <-ifelse(is.na(all.source$customer_f21_lag1), NA, all.source$f_21 / all.source$customer_f21_lag1)
# all.source$customer_f22_lag1_div <-ifelse(is.na(all.source$customer_f22_lag1), NA, all.source$f_22 / all.source$customer_f22_lag1)
# all.source$customer_f25_lag1_div <-ifelse(is.na(all.source$customer_f25_lag1), NA, all.source$f_25 / all.source$customer_f25_lag1)
# all.source$customer_f26_lag1_div <-ifelse(is.na(all.source$customer_f26_lag1), NA, all.source$f_26 / all.source$customer_f26_lag1)
# all.source$customer_f27_lag1_div <-ifelse(is.na(all.source$customer_f27_lag1), NA, all.source$f_27 / all.source$customer_f27_lag1)
# all.source$customer_f28_lag1_div <-ifelse(is.na(all.source$customer_f28_lag1), NA, all.source$f_28 / all.source$customer_f28_lag1)
all.source$customer_f30_lag1_div <-ifelse(is.na(all.source$customer_f30_lag1), NA, all.source$f_30 / all.source$customer_f30_lag1)
# all.source$customer_f32_lag1_div <-ifelse(is.na(all.source$customer_f32_lag1), NA, all.source$f_32 / all.source$customer_f32_lag1)
# all.source$customer_f34_lag1_div <-ifelse(is.na(all.source$customer_f34_lag1), NA, all.source$f_34 / all.source$customer_f34_lag1)
# all.source$customer_f35_lag1_div <-ifelse(is.na(all.source$customer_f35_lag1), NA, all.source$f_35 / all.source$customer_f35_lag1)
# all.source$customer_f36_lag1_div <-ifelse(is.na(all.source$customer_f36_lag1), NA, all.source$f_36 / all.source$customer_f36_lag1)
# all.source$customer_f37_lag1_div <-ifelse(is.na(all.source$customer_f37_lag1), NA, all.source$f_37 / all.source$customer_f37_lag1)
# all.source$customer_f38_lag1_div <-ifelse(is.na(all.source$customer_f38_lag1), NA, all.source$f_38 / all.source$customer_f38_lag1)
# all.source$customer_f39_lag1_div <-ifelse(is.na(all.source$customer_f39_lag1), NA, all.source$f_39 / all.source$customer_f39_lag1)
# all.source$customer_f40_lag1_div <-ifelse(is.na(all.source$customer_f40_lag1), NA, all.source$f_40 / all.source$customer_f40_lag1)
# all.source$customer_f41_lag1_div <-ifelse(is.na(all.source$customer_f41_lag1), NA, all.source$f_41 / all.source$customer_f41_lag1)

# cust.test <- all.source %>% filter(customer_id == "144112433441")
# head(cust.test$customer_f13_lag1_div,20)
# rm(cust.test)
all.source <- as.data.table(all.source)

# 
# cluster.columns <- c("f_3", "f_4", "f_6","f_8", 
#  "f_10","f_11"
# ,"f_12","f_13"
# ,"f_14","f_15"
# ,"f_17","f_18"
...
# 
# all.source <- as.data.frame(all.source)
# cl <- kmeans(all.source[,cluster.columns], 21)
# #plot(all.source[,cluster.columns], col = cl$cluster)
# #points(cl$centers, col = 1:2, pch = 8, cex = 2)
# 
# all.source$cluster <-  cl$cluster
# 
# all.source <- as.data.table(all.source)
