# -*- coding: utf-8 -*-
"""
Created on Wed Apr 18 18:52:30 2018
...
if WHERE=='kaggle':
	inpath  = 'C:/Users/SriPrav/Documents/R/47TalkingData'
	suffix = ''
	outpath = ''
	savepath = ''
elif WHERE=='gcloud':
...
def DO(frm,to,fileno):
    ...
    train_df = pd.read_csv(inDir+"/input/train.csv", parse_dates=['click_time'], skiprows=range(1,frm), nrows=to-frm, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])

    print('loading test data...')
    if debug:
        test_df = pd.read_csv(inDir+"/input/test.csv", nrows=100000, parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
    else:
