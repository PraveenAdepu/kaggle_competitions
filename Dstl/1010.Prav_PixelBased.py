...
train_wkt = pd.read_csv(inDir + '/input/train_wkt_v4.csv')

## Load sample submission
sample_submission = pd.read_csv(inDir + '/input/sample_submission.csv')

## Load grid size
grid_sizes = pd.read_csv(inDir + '/input/grid_sizes.csv')
...
#    filename = os.path.join(inDir,'input' ,'sources' ,'sixteen_band','sixteen_band', '{}_M.tif'.format(image_id))
    if image_type =='3':
        image = tiff.imread(inDir + '/input/sources/three_band/three_band/{}.tif'.format(image_id)).transpose([1, 2, 0])
        X = image.reshape(-1, 3).astype(np.float32) 
        image_size = image.shape[:2]
    else:
        X_P = tiff.imread(inDir + '/input/sources/sixteen_band/sixteen_band/{}_P.tif'.format(image_id))
        nomRow = X_P.shape[0]
        nomCol = X_P.shape[1]
        
#        Tracer()()
        imageMP = func_feature_scale(inDir + '/input/sources/sixteen_band/sixteen_band/{}_M.tif'.format(image_id), nomRow, nomCol)
        X_M = imageMP.reshape(-1, 8).astype(np.float32) 

        imageAP = func_feature_scale(inDir + '/input/sources/sixteen_band/sixteen_band/{}_A.tif'.format(image_id), nomRow, nomCol)
...
sample_submission_new = pd.read_csv(inDir + '/input/sample_submission_new.csv')
...
sub_file = inDir + '/submissions/Prav_Model10.csv'
pd_output[['ImageId','ClassType','MultipolygonWKT']].to_csv(sub_file, index = False)