...
FeatureExtraction_Folder = inDir + "/input/FeatureExtraction_00"
cvfolds = inDir + "/CVSchema/Prav_CVindices_5folds.csv"
sample_submission = inDir + "/input/sources/stage2_sample_submission.csv"
FeatureFiles = inDir + "/input/FeatureExtraction_00/%s.npy"
FeatureFiles_stg2 = inDir + "/input/FeatureExtraction_00_stg2/%s.npy"
...
test_x = np.array([np.mean(np.load(FeatureFiles_stg2 % str(id)), axis=0) for id in test_df['id'].tolist()])
    

    
MODEL_WEIGHTS_FILE = 'FE00_dsb_nn01_weights_stg2.h5'
...
#    m.fit(trn_x, trn_y, batch_size=32, nb_epoch=15, validation_data=(val_x, val_y), verbose=2,callbacks=callbacks)     
    m = nn_model(trn_x.shape[1])
    m.fit(trn_x, trn_y, batch_size=32, nb_epoch=8, validation_data=(val_x, val_y), verbose=2)
    pred_valid = [p[0] for p in m.predict(val_x)] #m.predict(val_x)
    pred_cv['cancer'] = pred_valid
    sub_valfile = 'C:/Users/SriPrav/Documents/R/19DSB2017/submissions/Prav.FE00.nn01.fold_stg2' + str(i) + '.csv'
    pred_cv.to_csv(sub_valfile, index=False)    
    pred_test =[p[0] for p in m.predict(test_x)] #
    test_df['cancer'] = pred_test
    
    sub_file = 'C:/Users/SriPrav/Documents/R/19DSB2017/submissions/Prav.FE00.nn01.fold_stg2' + str(i) + '-test' + '.csv'
    test_df.to_csv(sub_file, index=False)
    #os.remove('C:/Users/SriPrav/Documents/R/19DSB2017/FE00_dsb_nn01_weights.h5')
    del pred_cv
...
def fulltrain_nnet(bags):
    ...
    for i in range(1, bags+1):
        ...
        m.fit(fulltrain_x, fulltrain_y, batch_size=32, nb_epoch=8, validation_data=(fulltrain_x, fulltrain_y), verbose=2)
   
        pred_test = [p[0] for p in m.predict(test_x)] #m.predict(test_x) 
        test_df['cancer'] += pred_test
        test_df['cancer'].head() 
    test_df['cancer'] = test_df['cancer']/folds
    sub_file = 'C:/Users/SriPrav/Documents/R/19DSB2017/submissions/Prav.FE00.nn01_stg2.full' + str(i) + '-bags' + '.csv'
    test_df.to_csv(sub_file, index=False)
