...
merged_model.add(Merge([model1, model2, model3, model4], mode='concat')) # model1, model2,, model5, model6
...
checkpoint = ModelCheckpoint('dn5_weights.h5', monitor='val_loss', save_best_only=True, verbose=1)

merged_model.fit([x1, x2,x1, x2], y=y, batch_size=384, nb_epoch=10,
                 verbose=1, validation_split=0.2, shuffle=True, callbacks=[checkpoint])

merged_model.load_weights('C:/Users/SriPrav/Documents/R/23Quora/dn5_weights.h5')

preds = merged_model.predict([x11, x22,x11, x22])#,x11, x22
print(preds.shape)

out_df = pd.DataFrame({"test_id":test_df['test_id'], "is_duplicate":preds.ravel()})
sub_file = 'C:/Users/SriPrav/Documents/R/23Quora/submissions/Prav.deepnet05.csv'
