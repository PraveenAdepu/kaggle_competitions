# -*- coding: utf-8 -*-
"""
Created on Sun May 20 14:43:56 2018
...
trainingSet = pd.merge(trainingSet, Prav_train_FE03, how = 'left', on = 'item_id')
testingSet  = pd.merge(testingSet, Prav_test_FE03, how = 'left', on = 'item_id')

traintest_FE02 = pd.read_csv(inDir+"/input/traintest_FE_02.csv")

trainingSet = pd.merge(trainingSet, traintest_FE02, how = 'left', on = 'user_id')
testingSet  = pd.merge(testingSet, traintest_FE02, how = 'left', on = 'user_id')


feature_names = [c for c in trainingSet if c not in ['item_id', "user_id", "title", "description", "activation_date", "image", 'deal_probability','CVindices']]

trainingSet.fillna(-99, inplace=True)
testingSet.fillna(-99, inplace=True)
