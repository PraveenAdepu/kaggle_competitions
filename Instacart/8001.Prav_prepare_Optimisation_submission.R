...
#################################################################################################
xgb01.full           <- read_csv("./submissions/prav.xgb07.full.csv")
...
submission <- all.folds %>%
       group_by(order_id) %>%
       summarise(products = exact_F1_max_none(ensemble, product_id))
...
write.csv(submission, './submissions/prav.xgb07.ensemble-sub.csv', row.names=FALSE, quote = FALSE)