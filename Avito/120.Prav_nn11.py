...
train_df['description'].fillna(value='missing', inplace=True)
train_df['title'].fillna(value='missing', inplace=True)
train_df['param_1'].fillna(value='missing', inplace=True)
train_df['param_2'].fillna(value='missing', inplace=True)
train_df['param_3'].fillna(value='missing', inplace=True)

test_df['description'].fillna(value='missing', inplace=True)
test_df['title'].fillna(value='missing', inplace=True)
test_df['param_1'].fillna(value='missing', inplace=True)
test_df['param_2'].fillna(value='missing', inplace=True)
test_df['param_3'].fillna(value='missing', inplace=True)
...
train_df['title_desc_len_ratio'] = train_df['title_nchar']/train_df['description_nchar']
test_df['title_desc_len_ratio'] = test_df['title_nchar']/test_df['description_nchar']
...
train_test = train_test.replace(np.nan,0,regex=True) #nan and other missing values are mapped to -1
...
train_test['price'] = np.log1p(train_test['price']+0.001)
train_test['item_seq_number'] = np.log(train_test['item_seq_number'])

   
train_test['price'][train_test['price']==-np.inf] = 0 
train_test['price'].fillna(train_test.price.mean(), inplace=True)
train_test['price'].describe()
...
from keras.layers import Dense, Input, Embedding, Dropout, Flatten, Reshape,GRU,Bidirectional
...
#    model5.add(Bidirectional(GRU(num_lstm)))
    ...
    model5 = Sequential()
    model5.add(Embedding(nb_words, EMBEDDING_DIM, weights=[embedding_matrix], input_length=MAX_SEQUENCE_LENGTH,trainable=False))
    model5.add(Bidirectional(LSTM(num_lstm, dropout_W=rate_drop_lstm, dropout_U=rate_drop_lstm,return_sequences=True)))
...
def lstm_Bidirectional_selfEmbedding_model():      
...
#    model5.add(Bidirectional(GRU(num_lstm)))
    ...
    model5 = Sequential()
    model5.add(Embedding(nb_words, EMBEDDING_DIM, input_length=MAX_SEQUENCE_LENGTH,trainable=False)) # weights=[embedding_matrix],
    model5.add(Bidirectional(LSTM(num_lstm, dropout_W=rate_drop_lstm, dropout_U=rate_drop_lstm,return_sequences=True)))
    model5.add(Attention(MAX_SEQUENCE_LENGTH))
#    model5.add(Bidirectional(GRU(num_lstm)))
...
#    model5.add(Bidirectional(GRU(num_lstm)))
    ...
    model5 = Sequential()
    model5.add(Embedding(nb_words, EMBEDDING_DIM, input_length=MAX_SEQUENCE_LENGTH,trainable=False))
    model5.add(Bidirectional(GRU(num_lstm)))
#    model5.add(Attention(MAX_SEQUENCE_LENGTH))
#    model5.add(Bidirectional(GRU(num_lstm)))
...
def gru_Bidirectional_selfEmbedding_numeric_model():      
...
#    leaks_input = Input(shape=(leaks.shape[1],))
#    model7.add(Dense(num_dense/2, activation=act))
    model7.add(Dense(train_user_features.shape[1], input_dim = train_user_features.shape[1]))
    model7.add(PReLU())
    
    merged_model = Sequential()

    merged_model.add(Merge([ model_inp_reg,model_inp_city,model_inp_cat1,model_inp_cat2,model_inp_prm1,model_inp_prm2,model_inp_usr,model_inp_itype,model7], mode='concat'))
...
optim_type = 'Adam'
learning_rate = 1e-3
MODEL_WEIGHTS_FILE = 'Prav_nn11_weights.h5'
fullepochs = 12
...
#    train_data_sqnm_val  = train_data_sqnm[valindex]
    ...
    for j in range(1,nbags+1):
        ...
        model = gru_Bidirectional_selfEmbedding_numeric_model()
        model.summary()
...
#        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=patience, min_lr=1e-9, epsilon = 0.00001, verbose=VERBOSEFLAG)
        ...
        model.compile(optimizer=optim, loss='mean_squared_error', metrics =[root_mean_squared_error])
        
        model.fit([train_data_reg_train,train_data_city_train,train_data_cat1_train,train_data_cat2_train,train_data_prm1_train,train_data_prm2_train,train_data_usr_train,train_data_itype_train,train_user_features_train], labels_train, 
                  validation_data=([train_data_reg_val,train_data_city_val,train_data_cat1_val,train_data_cat2_val,train_data_prm1_val,train_data_prm2_val,train_data_usr_val,train_data_itype_val,train_user_features_val], labels_val), 
        ...
        bag_cv   = model.predict([train_data_reg_val,train_data_city_val,train_data_cat1_val,train_data_cat2_val,train_data_prm1_val,train_data_prm2_val,train_data_usr_val,train_data_itype_val,train_user_features_val], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
        pred_cv += model.predict([train_data_reg_val,train_data_city_val,train_data_cat1_val,train_data_cat2_val,train_data_prm1_val,train_data_prm2_val,train_data_usr_val,train_data_itype_val,train_user_features_val], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
        
        pred_test += model.predict([test_data_reg,test_data_city,test_data_cat1,test_data_cat2,test_data_prm1,test_data_prm2,test_data_usr,test_data_itype,test_user_features], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
    ...
    sub_valfile = inDir+'/submissions/Prav.nn11.fold' + str(i) + '.csv'    
    ...
    sub_file = inDir+'/submissions/Prav.nn11.fold' + str(i) + '-test' + '.csv'
...
#        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=patience, min_lr=1e-9, epsilon = 0.00001, verbose=VERBOSEFLAG)
    ...
    pred_test = pred_test[['item_id','deal_probability']]
    pred_test['deal_probability'] = pred_test['deal_probability'].clip(0.0, 1.0)
    sub_file = inDir+'/submissions/Prav.nn11.full' + '.csv'
    pred_test.to_csv(sub_file, index=False)
