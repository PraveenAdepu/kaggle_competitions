...
X = X4[:,:-horizon_periods].copy()
Y = X4[:,-horizon_periods:].copy()

X5 = X3[:,3:]

testingSet = X5[:,horizon_periods:].copy()
...
for nb_sample in range(0,testingSet.shape[0]):
    print(nb_sample)
    current_sample = []
    current_sample = testingSet[nb_sample]
    current_sample_format = []
    current_sample_format = np.atleast_2d(np.array([current_sample[start:start + features_size] for start in range(0, current_sample.shape[0] - features_size)]))
    X_test.append(current_sample_format)
...
learning_rate = 1e-3
ModelName = 'CNN02'
MODEL_WEIGHTS_FILE = inDir + '/Prav_CNN_02.h5'
...
input_nb_series = X_train.shape[2]
filter_length = 5
nb_filter = 4
output_nb_series = Y.shape[1]
...
#    
    ...
    pred_valid = model.predict(X_valid,batch_size=batch_size,verbose = 1)    
    
    pred_valid = pred_valid * maxi
...
#    X_valid = X_valid[0:math.floor(np.ceil((np.ceil(len(X_valid) / batch_size)-1)*batch_size)),]
#    
model.fit_generator( generator=batch_generator_train(X_train, Y ,batch_size),                            
