...
anscols = paste("lag41", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 41, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag42", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 42, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag43", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 43, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag44", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 44, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag45", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 45, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag46", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 46, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag47", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 47, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag48", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 48, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag49", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 49, NA, "lag"), .SDcols=cols, by=Patient_ID]

anscols = paste("lag50", cols, sep="_")
all_data_full[, (anscols) := shift(.SD, 50, NA, "lag"), .SDcols=cols, by=Patient_ID]
...
names(build_set)
unique(build_set$StateCode)
length(unique(build_set$postcode.x))

##################################################################################################################
OriginalFeatures <- c("Patient_ID","Drug_ID","ChronicIllness", "DispenseDate","year_of_birth","gender","StateCode","postcode.x")
...
build_set$StateCode <- as.integer(as.factor(build_set$StateCode))
...
head(build_set$Diabetes)
build_set$DiabetesFlag <- ifelse(build_set$Diabetes > 0, 1, 0)
...
feature.names     <- names(trainingSet[,-which(names(trainingSet) %in% c("Patient_ID", "DispenseDate" ,"DiabetesDispense", "CVindices"                               
                                                 
                    , "MasterProductCode"                          ,  "MasterProductFullName"                       
                    , "BrandName"                                  ,  "FormCode"                                    
                    , "StrengthCode"                               ,  "PackSizeNumber"                              
                    , "GenericIngredientName"                      ,  "EthicalSubCategoryName"                      
                    , "EthicalCategoryName"                        ,  "ManufacturerCode"                            
                    , "ManufacturerName"                           ,  "ManufacturerGroupID"                         
                    , "ManufacturerGroupCode"                      ,  "ChemistListPrice"                            
                    , "ATCLevel5Code"                              ,  "ATCLevel4Code"                               
                    , "ATCLevel3Code"                              ,  "ATCLevel2Code"                               
                    , "ATCLevel1Code"                              ,"OrderRank" ))])
...
write.csv(testfull_predictions, './submissions/prav.xgb5.full.csv', row.names=FALSE, quote = FALSE)
...
val_sub <- read.csv("./submissions/prav.xgb5.full.csv")
...
write.csv(Final_sub,  './submissions/Prav_xgb04_rank.csv', row.names=FALSE, quote = FALSE)
#########################################################################################################
