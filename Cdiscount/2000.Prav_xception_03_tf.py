...
def model_Xception(num_classes):
...
model = model_Xception(num_classes=num_classes) 
...
train_img_gen = ImageDataGenerator( rescale=1./255,
                                    ...
                                    horizontal_flip=True) #Configure as you want
train_gen = BinFileIterator('train.bin', img_generator=train_img_gen,  samples=2000000,
                 target_size=(180,180), 
                 batch_size=batch_size)  

val_img_gen = ImageDataGenerator(rescale=1./255) #Configure as you want
...
model.fit_generator( generator=train_gen,                            
                     ...
                     callbacks = callbacks,
                     validation_data=val_gen, 
                     validation_steps = math.ceil(25 / batch_size), 
                     max_q_size=10,
                     workers = 4,
                     verbose = VERBOSEFLAG 
                  )
...
#train_gen = BinFileIterator('train.bin', img_generator=train_img_gen,  samples=len(train_images_df),
#                 target_size=(180,180), 
#                 batch_size=100)  
#
#val_img_gen = ImageDataGenerator() #Configure as you want
#val_gen = BinFileIterator('val.bin', img_generator=val_img_gen,  samples=len(val_images_df),
#                 target_size=(180,180), 
#                 batch_size=100) 
#
## To train the model:
#model.fit_generator(train_gen,
#                    steps_per_epoch = len(train_images_df)/100,   #num_train_images // batch_size,
#                    epochs = 2,
#                    validation_data = val_gen,
#                    validation_steps = len(val_images_df)/100,  #num_val_images // batch_size,
#                    workers = 4)
  #####################################################################################################################    