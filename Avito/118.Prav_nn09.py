...
optim_type = 'Adam'
learning_rate = 1e-3
MODEL_WEIGHTS_FILE = 'Prav_nn09_weights.h5'
fullepochs = 12
...
#    train_data_sqnm_val  = train_data_sqnm[valindex]
    ...
    for j in range(1,nbags+1):
        ...
        model = lstm_Bidirectional_selfEmbedding_model()
...
#        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=patience, min_lr=1e-9, epsilon = 0.00001, verbose=VERBOSEFLAG)
        ...
        bag_cv   = model.predict([data_1_val, train_data_reg_val,train_data_city_val,train_data_cat1_val,train_data_cat2_val,train_data_prm1_val,train_data_prm2_val,train_data_usr_val,train_data_itype_val,train_user_features_val], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
        pred_cv += model.predict([data_1_val, train_data_reg_val,train_data_city_val,train_data_cat1_val,train_data_cat2_val,train_data_prm1_val,train_data_prm2_val,train_data_usr_val,train_data_itype_val,train_user_features_val], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
        
        pred_test += model.predict([test_data_1, test_data_reg,test_data_city,test_data_cat1,test_data_cat2,test_data_prm1,test_data_prm2,test_data_usr,test_data_itype,test_user_features], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
    ...
    sub_valfile = inDir+'/submissions/Prav.nn09.fold' + str(i) + '.csv'    
    ...
    sub_file = inDir+'/submissions/Prav.nn09.fold' + str(i) + '-test' + '.csv'
...
#        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=patience, min_lr=1e-9, epsilon = 0.00001, verbose=VERBOSEFLAG)
    ...
    pred_test = pred_test[['item_id','deal_probability']]
    pred_test['deal_probability'] = pred_test['deal_probability'].clip(0.0, 1.0)
    sub_file = inDir+'/submissions/Prav.nn09.full' + '.csv'
    pred_test.to_csv(sub_file, index=False)
