...
Fulltrain$lmtarget <- ifelse(Fulltrain$fault_severity== 0,0, 1)
...
target <- ifelse(Fulltrain$fault_severity== 0,0, 1)
...
head(train_lr)
# saving predictions
train_lr <- data.frame("id"=Fulltrain$id,  "pred_lr"=train_lr$pred_lr)
test_lr <- data.frame("id"=Fulltest$id, "pred_lr"=test_lr$pred_lr)

Fulltrain <- merge(Fulltrain, train_lr,by=(c("id" ))    ,all.x = T )
Fulltest <- merge(Fulltest, test_lr,by=(c("id" ))    ,all.x = T )
