...
validation.pred.columns <- c("ID","Script Directions","CVindices","ScriptQty")
  ...
  
{
  ...
  dtrain <-xgb.DMatrix(data=data.matrix(X_build[, feature.names]),label=X_build$ScriptQty)
  dval   <-xgb.DMatrix(data=data.matrix(X_val[, feature.names])  ,label=X_val$ScriptQty)
  ...
  names(X_val_pred) <- "ScriptQtyProbability"
  if(i == 1)
...
OOF_ScriptQty <- OOF_preds
rm(OOF_preds)
head(OOF_ScriptQty)

sum(OOF_ScriptQty$ScriptQtyProbability)
sum(OOF_ScriptQty$ScriptQty)
# Full training
...
# dtrain<-xgb.DMatrix(data=data.matrix(trainingSet[, feature.names]),label=trainingSet$ScriptQty)
...
# testfull_predictions  <- data.frame(id=testingSet$id, ScriptQtyProbability = round(predfull_test))
