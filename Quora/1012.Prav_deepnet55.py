...
features_to_use = cols = [col for col in train_df.columns if col not in ['id','qid1_x','qid2_x','question1_x', 'question2_x','is_duplicate_x','CVindices','test_id'
                                                                             ,'qid1_y','qid2_y','question1_y', 'question2_y','is_duplicate_y'
                                                                       ,'q1_2pairwords_matched_q2_ratio','z_tfidf_word_match', 'q1_hash',
                                                                       'q2_hash']] 
leaks = train_df[features_to_use]
                     
test_leaks = test_df[features_to_use]
...
def lstm_model():
    ...
    model1.add(TimeDistributed(Dense(num_lstm, activation='relu')))
    model1.add(Lambda(lambda x: K.sum(x, axis=1), output_shape=(num_lstm,)))
    ...
    model2.add(TimeDistributed(Dense(num_lstm, activation='relu')))
    model2.add(Lambda(lambda x: K.sum(x, axis=1), output_shape=(num_lstm,)))
...
hist = model.fit([data_1_train, data_2_train,data_1_train, data_2_train,data_1_train, data_2_train, leaks_train], labels_train, \
        validation_data=([data_1_val, data_2_val,data_1_val, data_2_val,data_1_val, data_2_val, leaks_val], labels_val, weight_val), \
...
preds = model.predict([test_data_1, test_data_2,test_data_1, test_data_2,test_data_1, test_data_2, test_leaks], batch_size=8192, verbose=1)
preds += model.predict([test_data_2, test_data_1,test_data_2, test_data_1,test_data_2, test_data_1, test_leaks], batch_size=8192, verbose=1)
preds /= 2
