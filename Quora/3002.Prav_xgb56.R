...
# train_df$word_match_q1_freq_ration <- ifelse(is.finite(train_df$word_match/train_df$q1_freq),train_df$word_match/train_df$q1_freq,0)
# train_df$q2_q1_freq_ration <- ifelse(is.finite(train_df$q2_freq/train_df$q1_freq),train_df$q2_freq/train_df$q1_freq,0)
# train_df$word_match_tfidf_ration <- ifelse(is.finite(train_df$word_match/train_df$tfidf_word_match),train_df$word_match/train_df$tfidf_word_match,0)
# train_df$q3g_2g_ration <- ifelse(is.finite(train_df$q_3gram_jaccard/train_df$q_2gram_jaccard),train_df$q_3gram_jaccard/train_df$q_2gram_jaccard,0)
# train_df$q3g_5g_ration <- ifelse(is.finite(train_df$q_3gram_jaccard/train_df$q_5gram_jaccard),train_df$q_3gram_jaccard/train_df$q_5gram_jaccard,0)
# 
# score(train_df$word_match_q1_freq_ration, train_df$is_duplicate, metric)
# score(train_df$q2_q1_freq_ration, train_df$is_duplicate, metric)
# score(train_df$word_match_tfidf_ration, train_df$is_duplicate, metric)
# score(train_df$q3g_2g_ration, train_df$is_duplicate, metric)
# score(train_df$q3g_5g_ration, train_df$is_duplicate, metric)
# 
# score(train_df$q_3gram_jaccard, train_df$is_duplicate, metric)
# score(train_df$q_5gram_jaccard, train_df$is_duplicate, metric)


Impfeatures <- c(                     "word_match"
                                      ,"q2_freq"
                                      ,"q1_freq"
                                      ,"tfidf_word_match"
                                      ,"q_MatchedWords_ratio_to_q_pmin"
...
feature.names <- setdiff(feature.names, Impfeatures)
##################################################################################################################################
##################################################################################################################################
  ...
  
{
  ...
  if(i == 1)
  {
    write.csv(val_predictions,  './submissions/prav.xgb57.fold1.csv', row.names=FALSE, quote = FALSE)
    write.csv(test_predictions, './submissions/prav.xgb57.fold1-test.csv', row.names=FALSE, quote = FALSE)
  }
  if(i == 2)
  {
    write.csv(val_predictions,  './submissions/prav.xgb57.fold2.csv', row.names=FALSE, quote = FALSE)
    write.csv(test_predictions, './submissions/prav.xgb57.fold2-test.csv', row.names=FALSE, quote = FALSE)
  }
  if(i == 3)
  {
    write.csv(val_predictions,  './submissions/prav.xgb57.fold3.csv', row.names=FALSE, quote = FALSE)
    write.csv(test_predictions, './submissions/prav.xgb57.fold3-test.csv', row.names=FALSE, quote = FALSE)
  }
  if(i == 4)
  {
    write.csv(val_predictions,  './submissions/prav.xgb57.fold4.csv', row.names=FALSE, quote = FALSE)
    write.csv(test_predictions, './submissions/prav.xgb57.fold4-test.csv', row.names=FALSE, quote = FALSE)
  }
  if(i == 5)
  {
    write.csv(val_predictions,  './submissions/prav.xgb57.fold5.csv', row.names=FALSE, quote = FALSE)
    write.csv(test_predictions, './submissions/prav.xgb57.fold5-test.csv', row.names=FALSE, quote = FALSE)
  }
...
write.csv(testfull_predictions, './submissions/prav.xgb57.full.csv', row.names=FALSE, quote = FALSE)
...
names = dimnames(train_df[,feature.names])[[2]]
...
sub_valfile = 'C:/Users/SriPrav/Documents/R/23Quora/submissions/Prav.xgb57.fold' + str(i) + '.csv'
...
sub_file = 'C:/Users/SriPrav/Documents/R/23Quora/submissions/Prav.xgb57.fold' + str(i) + '-test' + '.csv'
...
sub_file = 'C:/Users/SriPrav/Documents/R/23Quora/submissions/Prav.xgb57.full' + '.csv'
