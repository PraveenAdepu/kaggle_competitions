...
train_ts <- ts(x0, frequency = 7)

cl <- makeCluster(28)
registerDoParallel(cl)
fcst_matrix <- foreach(i=1:nrow(train_ts),.combine=rbind, .packages=c("forecastHybrid")) %dopar% {
  fcst_matrix <- expm1(forecast(hybridModel(log1p(train_ts[i,]),models = "ae", weights="insample"
                                            , errorMethod = c("RMSE"),num.cores = 30, verbose = TRUE),h=39)$mean)
}
stopCluster(cl)
...
colnames(fcst_matrix) <- as.character(seq(from = as.Date("2017-03-12"), 
                                          to = as.Date("2017-04-19"), 
                                          by = 'day'))
pred_val <- as.data.frame( fcst_matrix)
...
write.csv(pred_validation, "./submissions/Prav_forecastHybrid_validation_preds.csv",row.names = FALSE, quote = FALSE)

pred_validation_adjust <- pred_validation %>% filter(visitors != 0) # 27,728
score(log1p(pred_validation_adjust$visitors), log1p(pred_validation_adjust$visitors_pred),"rmse")
#0.80
...
train_ts <- ts(train_wide, frequency = 7)
...
pred_test <- as.data.frame(fcst_matrix)
...
write.csv(pred_test_long, "./submissions/Prav.forecastHybrid01full-test.csv",row.names = FALSE, quote = FALSE)
