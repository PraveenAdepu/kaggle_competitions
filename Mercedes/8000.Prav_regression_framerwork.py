...
def train_xgboost_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,xgbParameters, num_rounds):
    ...
    xgbbuild = xgb.DMatrix(X_build[feature_names], label=X_build['is_duplicate'])
    xgbval = xgb.DMatrix(X_valid[feature_names], label=X_valid['is_duplicate'])
...
def fulltrain_xgboost_regression(trainingSet, testingSet,feature_names,nbags,ModelName,xgbParameters, num_rounds):
    ...
    xgbtrain = xgb.DMatrix(trainingSet[feature_names], label=trainingSet['is_duplicate'])
...
def train_lgbm_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,lgbmParameters, lgbm_num_rounds):
    ...
    lgbmbuild = lightgbm.Dataset(X_build[feature_names], X_build['is_duplicate'])
    lgbmval   = lightgbm.Dataset(X_valid[feature_names], X_valid['is_duplicate'])
    ...
    for j in range(1,nbags+1):
        ...
        model = lightgbm.train(lgbmParameters, lgbmbuild,lgbm_num_rounds,valid_sets=lgbmval,verbose_eval = 500
...
def fulltrain_lgbm_regression(trainingSet, testingSet,feature_names,nbags,ModelName,lgbmParameters, lgbm_num_rounds):
    ...
    lgbmtrain = lightgbm.Dataset(trainingSet[feature_names], trainingSet['is_duplicate'])

    predfull_test = np.zeros(testingSet.shape[0]) 
    for j in range(1,nbags+1):
        print('bag ', j , ' Processing')
        lgbmParameters['seed'] =  current_seed + j           
        fullmodel = lightgbm.train(lgbmParameters, lgbmtrain,full_lgbm_num_rounds,verbose_eval = 500
                                  #,early_stopping_rounds=early_stopping_rounds
                                  )
    ...
    if RegressionModelName in ["rf","ada","gbdt","et","lr","lsvc","knn"]:    
        ...
        for j in range(1,nbags+1):
            ...
            if RegressionModelName == "rf":
                RegressionModel = RandomForestRegressor(n_estimators=2000, max_depth=8, n_jobs=20, random_state=bag_seed, max_features="auto",verbose=1)
            ...
            if RegressionModelName == "gbdt":
                RegressionModel = GradientBoostingRegressor(n_estimators=1500,learning_rate=0.04,  subsample=0.8, random_state=bag_seed,max_depth=5,verbose=1)
            if RegressionModelName == "et":
                RegressionModel = ExtraTreesRegressor(n_estimators=2000, max_depth=6, max_features="auto", n_jobs=20, random_state=bag_seed,verbose=1)
            ...
            RegressionModel.fit(X_build[feature_names],X_build['is_duplicate'])
...
def fulltrain_regression(RegressionModelName,trainingSet, testingSet,feature_names,nbags,ModelName,current_seed):
    if RegressionModelName in ["rf","ada","gbdt","et","lr","lsvc","knn"]:    
        ...
        for j in range(1,nbags+1):
            ...
            bag_seed = current_seed + j
            if RegressionModelName == "rf":
                RegressionModel = RandomForestRegressor(n_estimators=2200, max_depth=8, n_jobs=20, random_state=bag_seed, max_features="auto",verbose=1)
            if RegressionModelName == "ada":
            ...
            if RegressionModelName == "et":
                RegressionModel = ExtraTreesRegressor(n_estimators=2200, max_depth=6, max_features="auto", n_jobs=20, random_state=bag_seed,verbose=1)
            if RegressionModelName == "lr":
                RegressionModel = LinearRegression(n_jobs=20) 
            if RegressionModelName == "knn":
                RegressionModel = KNeighborsRegressor(n_neighbors=2,n_jobs=20)
                
            RegressionModel.fit(trainingSet[feature_names],trainingSet['is_duplicate'])    
