# -*- coding: utf-8 -*-
"""
Created on Sat Dec 24 09:11:39 2016
...
with open(data_path+"leak.csv") as infile:
	...
	for ind, row in enumerate(doc):
		...
		leak_uuid_dict[document_id] = set(row[1].split(' '))
		if ind%100000==0:
...
#with open(trainLeak, 'w') as outfile:
#    outfile.write('display_id,ad_id,leak\n')
#    for t, disp_id, ad_id, leak in data(train, D):  # data is a generator
#         outfile.write('%s,%s,%s\n' % (disp_id, ad_id, leak))
#         if t%1000000 == 0:
...
with open(testLeak, 'w') as outfile:
    outfile.write('display_id,ad_id,leak\n')
    for t, disp_id, ad_id, leak in data(test, D):
        outfile.write('%s,%s,%s\n' % (disp_id, ad_id, leak))
        if t%1000000 == 0:
            print("Processed : ", t, datetime.now())
#        if t ==100000:
