...
densenet121_weights = 'C:/Users/SriPrav/Documents/R' + '/imagenet_models/densenet121_weights_th.h5'
MODEL_WEIGHTS_FILE = inDir + '/Densenet_CCN01_weights.h5'
...
def densenet121_model(img_rows, img_cols, color_type=1, nb_dense_block=4, growth_rate=32, nb_filter=64, reduction=0.5, dropout_rate=0.0, weight_decay=1e-4, num_classes=None):
    ...
    weights_path = densenet121_weights
...
def create_submission(predictions, test_id, info):
    ...
    sub_file = 'submissions\Prav.densenet121.stg1and2.CNN01_' + info + '_' + str(now.strftime("%Y-%m-%d-%H-%M")) + '.csv'
...
#        model = create_model()# Load our model
        model = densenet121_model(ROWS, COLUMNS, CHANNELS, num_classes=3)
...
#              )
        model.fit_generator( train_datagen.flow( X_train, Y_train, batch_size = batch_size,shuffle=True),
                             samples_per_epoch = len(X_train), nb_epoch = nb_epoch, callbacks = callbacks,
                             validation_data=valid_datagen.flow(X_valid, Y_valid, batch_size=batch_size), 
                             nb_val_samples=X_valid.shape[0], verbose = VERBOSEFLAG )
        model.load_weights(MODEL_WEIGHTS_FILE)
#        predictions_valid = model.predict(X_valid.astype('float32')* (1./255), batch_size=batch_size, verbose=VERBOSEFLAG)
        predictions_valid = model.predict_generator(valid_datagen.flow(X_valid, batch_size=batch_size,shuffle=False),val_samples=X_valid.shape[0])
...
#        test_prediction = model.predict(test_data.astype('float32')* (1./255), batch_size=batch_size, verbose=VERBOSEFLAG)
        test_prediction = model.predict_generator(test_datagen.flow(test_data, batch_size=batch_size,shuffle=False),val_samples=test_data.shape[0])
