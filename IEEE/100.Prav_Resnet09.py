...
images_train1 = images_train1[images_train1['image_patch_rank']<=25]

images_train2 = pd.read_csv(inDir + '/input/Prav_5folds_CVindices_v51.csv')
images_train2 = images_train2[images_train2['image_patch_rank']<=7]
...
i=1
def dlnet(i):
    MODEL_WEIGHTS_FILE = inDir + '/Prav_01_Resnet09_'+str(i)+'.h5'
...
#                                 validation_data=batch_generator_X_valid(images_valid,X_valid,y_valid, batch_size, shuffle=True), 
#                                 #nb_val_samples=len(valid_index), 
#                                 validation_steps = math.ceil((len(images_valid)/2) / batch_size), #int(len(valid_index)/float(batch_size)),
#                                 max_q_size=10,
#                                 verbose = VERBOSEFLAG 
#                      )
#        
