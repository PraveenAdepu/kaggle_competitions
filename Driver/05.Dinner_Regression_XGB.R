...
validation.pred.columns <- c("ID","Script Directions","CVindices","Dinner")
  ...
  
{
  ...
  dtrain <-xgb.DMatrix(data=data.matrix(X_build[, feature.names]),label=X_build$Dinner)
  dval   <-xgb.DMatrix(data=data.matrix(X_val[, feature.names])  ,label=X_val$Dinner)
  ...
  names(X_val_pred) <- "DinnerProbability"
  if(i == 1)
...
OOF_Dinner <- OOF_preds
rm(OOF_preds)
head(OOF_Dinner)

sum(OOF_Dinner$DinnerProbability)
sum(OOF_Dinner$Dinner)
# Full training
...
# dtrain<-xgb.DMatrix(data=data.matrix(trainingSet[, feature.names]),label=trainingSet$Dinner)
...
# testfull_predictions  <- data.frame(id=testingSet$id, DinnerProbability = round(predfull_test))
