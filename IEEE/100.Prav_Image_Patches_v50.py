...
images_train = pd.read_csv(inDir + '/input/Prav_5folds_CVindices_externaldata.csv')
...
def load_test_frombatch(images_batch):   
    for fl in images_batch.image_path.values:
        try:
            img = get_im_cv2(fl)  
            current_image_patches = image_patches(img,224,224,3)
            new_fl = fl.replace('train_50','train_51')
            for i in range(0,len(current_image_patches)):
                new_path = new_fl.replace('.jpg','_{}.jpg'.format(i))
                scipy.misc.imsave(new_path, current_image_patches[i])
        except:
...
images_train_v5 = pd.read_csv(inDir + '/input/images_train_externaldata_patches.csv')

images_train_v5.head()

images_train_images = images_train_v5['image_name'].str.split('_', expand=True)

images_train_images.columns = ['image_name1','image_name2','image_name3','extension']
...
images_train_images['image_name'] = images_train_images['image_name1']+'_'+images_train_images['image_name2']+'_'+images_train_images['image_name3']+'.'+images_train_images1['extensions']
...
images_train_v5_details['image_patch_rank'] = images_train_v5_details.groupby(['CVindices','image_name']).cumcount()+1


images_train_v5_details[["image_path","image_name_patch","image_category","image_name","extension","image_patch_number","CVindices","image_patch_rank"]].to_csv(inDir+"/input/Prav_5folds_CVindices_v51.csv", index=False)


images_train_v5_details[images_train_v5_details['image_patch_rank']<=10].groupby(['CVindices','image_category'])[['image_category']].size()
