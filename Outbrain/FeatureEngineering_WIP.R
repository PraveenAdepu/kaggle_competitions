...
head(documents_meta)
length(documents_meta$document_id)
length(unique(documents_meta$document_id))

documents_categories <- fread("./input/documents_categories.csv") 
names(documents_categories)

document_category_features <- sqldf("SELECT document_id, count(*) CategoryCount, 
                                                        Max(confidence_level) MaxCategoryLevel, 
                                                        Min(confidence_level) MinCategoryLevel,
                                                        Avg(confidence_level) MeanCategoryLevel 
                                    FROM documents_categories 
                                    GROUP BY document_id ")

length(documents_categories$document_id)
...
document_entities_features <- sqldf("SELECT document_id, count(*) EntitiesCount, 
                                                        Max(confidence_level) MaxEntitiesLevel, 
                                                        Min(confidence_level) MinEntitiesLevel,
                                                        Avg(confidence_level) MeanEntitiesLevel 
...
document_topics_features <- sqldf("SELECT document_id, count(*) TopicsCount, 
                                                        Max(confidence_level) MaxTopicsLevel, 
                                    Min(confidence_level) MinTopicsLevel,
                                    Avg(confidence_level) MeanTopicsLevel 
...
head(train_test,2)
head(events,2)
head(promoted_content,2)
head(documents_meta,2)
# documents_meta$Date <- strptime(documents_meta$publish_time, ,format="&m/%d/%Y  %H:$M")
# documents_meta$Date <-  as.POSIXct(documents_meta$publish_time, tz="UTC")
# 
# head(documents_meta)
# sapply(documents_meta, class)
# 
# day(t.lub)
# minute(t.lub)/60
...
train_test <- left_join(train_test, promoted_content, by = c("ad_id","document_id"))

train_test <- left_join(train_test, documents_meta, by = "document_id")

# train_test <- left_join(train_test, document_category_features, by = "document_id")
# train_test <- left_join(train_test, document_entities_features, by = "document_id")
# train_test <- left_join(train_test, document_topics_features, by = "document_id")
