...
def create_submission(predictions, test_id, info):
    ...
    sub_file = 'submissions\Prav.densenet161.CNN03_all_Aug_' + info + '_' + str(now.strftime("%Y-%m-%d-%H-%M")) + '.csv'
...
#              )
        model.fit_generator(train_datagen.flow(X_train, Y_train, batch_size=batch_size),
                    samples_per_epoch=len(X_train), nb_epoch=nb_epoch,validation_data=valid_datagen.flow(X_valid, Y_valid, batch_size=batch_size)
                    ,nb_val_samples=len(X_valid)
                    ,verbose=VERBOSEFLAG,callbacks=callbacks)
...
def run_cross_validation_process_test(info_string, models):
    ...
    for i in range(nfolds):
        ...
        for aug in range(test_aug):
            if aug == 0:
                test_prediction = model.predict_generator(test_datagen.flow(test_data, batch_size=batch_size),val_samples=test_data.shape[0] - test_data.shape[0]%batch_size)
            else:
                test_prediction += model.predict_generator(test_datagen.flow(test_data, batch_size=batch_size),val_samples=test_data.shape[0] - test_data.shape[0]%batch_size)
        test_prediction /= test_aug
