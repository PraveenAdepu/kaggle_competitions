...
svmGrid <- data.frame(C=c(0.01),sigma = c(1))

system.time(svm.tune <- train(x=Fulltrain[,featureNames],y=Fulltrain$fault_severity,
                               #fault_severity ~., 
...
# Training error rate
confusionMatrix(predict(svm.tune, Fulltrain[,featureNames]), Fulltrain$fault_severity)
# Accuracy : 0.6931

# predictedClasses <- predict(svm.c, Fulltest[,TestfeatureNames] )
predictedProbs   <- predict(svm.tune, Fulltest[,TestfeatureNames], type = "prob")
...
write.csv(prediction, "submission27.csv", quote=FALSE, row.names = FALSE)
