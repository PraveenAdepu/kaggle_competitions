...
#    fulltrain_xgboost_regression(trainingSet, testingSet,feature_names,nbags,xgbModelName,xgbParameters, xgb_num_rounds,current_seed,verboseeval)
    ...
    model_results = []
    model_results.append(lgbmModelName)  
    for i in range(1, folds+1):
        fold_score = train_lgbm_regression(trainingSet, testingSet,feature_names,i,nbags,lgbmModelName,lgbm_params, lgbm_num_round,current_seed,verboseeval)
        model_results.append(fold_score)
    LB = 0
    model_results.append(LB)
    ScoreDiff = 0
    model_results.append(ScoreDiff)
    model_results.append(lgbm_params)
