...
image_Exiffeatures = inDir + "/input/Prav_ImageExif_02_features.csv"
image_Exiffeatures = pd.read_csv(image_Exiffeatures)

#image_Exiffeatures["maxDateTimeOriginal"].describe()
#image_Exiffeatures["maxDateTimeOriginal"].head(10)
#image_Exiffeatures["maxDateTimeOriginal"] = image_Exiffeatures["maxDateTimeOriginal"].fillna('2016-04-01 22:12:41')
#image_Exiffeatures["maxDateTimeOriginal"] = pd.to_datetime(image_Exiffeatures["maxDateTimeOriginal"])
...
train_DescriptionSentimentFeatures = inDir + "/input/Prav_train_DescriptionSentimentFeatures.csv"
train_DescriptionSentimentFeatures = pd.read_csv(train_DescriptionSentimentFeatures)

test_DescriptionSentimentFeatures = inDir + "/input/Prav_test_DescriptionSentimentFeatures.csv"
test_DescriptionSentimentFeatures = pd.read_csv(test_DescriptionSentimentFeatures)

train_df = pd.merge(train_df, train_DescriptionSentimentFeatures, how = 'left', on = 'listing_id')
test_df = pd.merge(test_df, test_DescriptionSentimentFeatures, how = 'left', on = 'listing_id')


listing_DuplicateImageCount = inDir + "/input/Prav_listingDuplicateImageCounts.csv"
listing_DuplicateImageCount = pd.read_csv(listing_DuplicateImageCount)


train_df = pd.merge(train_df, listing_DuplicateImageCount, how = 'left', on = 'listing_id')
test_df = pd.merge(test_df, listing_DuplicateImageCount, how = 'left', on = 'listing_id')



#train_images = train_df[["listing_id","photos"]]
#train_images.head(5)

#b = pd.DataFrame(train_images.photos.str.split(',').tolist(), index=train_images.listing_id).stack()
#b = b.reset_index()[[0, 'listing_id']] # var1 variable is currently labeled 0
#b.columns = ['photo', 'listing_id'] # renaming var1
#
#b.head(5)
...
#train_df["maxDateTimeOriginal"].describe()
#
#train_df["maxDateTimeOriginal"] = pd.to_datetime(train_df["maxDateTimeOriginal"])
#
#
#train_df["maxDateTimeOriginal"] = train_df["maxDateTimeOriginal"].fillna('2016-04-01 22:12:41')
#
#train_df["maxDateTimeOriginal"] = train_df["maxDateTimeOriginal"].fillna(test_df["created"].min(), inplace=True)
#train_df["maxDateTimeOriginal"] = train_df["maxDateTimeOriginal"].fillna(datetime(2016, 4, 1))
#train_df["maxDateTimeOriginal"] = pd.to_datetime(train_df["maxDateTimeOriginal"])
#
#test_df["maxDateTimeOriginal"].head(5)
#test_df["maxDateTimeOriginal"] = test_df["maxDateTimeOriginal"].fillna(test_df["created"].min())
#test_df["maxDateTimeOriginal"] = pd.to_datetime(test_df["maxDateTimeOriginal"])
#
...
train_df["DuplicateImage_Ratio"] = train_df["DuplicateImageCount"] / train_df["num_photos"].apply(lambda x: 1 if x == 0 else x) 
test_df["DuplicateImage_Ratio"] =  test_df["DuplicateImageCount"] / test_df["num_photos"].apply(lambda x: 1 if x == 0 else x) 

#train_df["ImageToPostpassed"] = train_df["maxDateTimeOriginal"] - train_df["created"]
#test_df["ImageToPostpassed"] = test_df["maxDateTimeOriginal"] - test_df["created"]
...
features_to_use.extend(["price_t","num_photos", "num_features", "num_description_words"
                        ...
                        ,'street', 'avenue', 'east', 'west', 'north', 'south','distance_to_city','DuplicateImage_Ratio']) # ,"passed" ,'manager_freq'
categorical = ["display_address", "manager_id", "building_id","street_address","maxMake"]
...
def train_xgboost(i):
    ...
    sub_valfile = 'C:/Users/SriPrav/Documents/R/21Rental/submissions/Prav.xgb121.fold' + str(i) + '.csv'
    ...
    sub_file = 'C:/Users/SriPrav/Documents/R/21Rental/submissions/Prav.xgb121.fold' + str(i) + '-test' + '.csv'
...
def fulltrain_xgboost(bags):
    ...
    sub_file = 'C:/Users/SriPrav/Documents/R/21Rental/submissions/Prav.xgb121.full' + '.csv'
