...
X_valid = pd.read_csv(inDir + '/input/X_valid.csv')


X_build = X_build.fillna(-1)
X_valid = X_valid.fillna(-1)

col = [c for c in X_build if c not in ['id', 'air_store_id','visit_date','visitors','air_genre_count'
                                       ,'ar_reserve_datetime_diff_min',
 'ar_reserve_datetime_diff_max',
 'ar_reserve_datetime_diff_mean',
 'ar_reserve_datetime_diff_std','hr_reserve_datetime_diff_min',
 'hr_reserve_datetime_diff_max',
 'hr_reserve_datetime_diff_mean',
 'hr_reserve_datetime_diff_std','std_visitors','hr_reserve_visitors_count','ar_reserve_visitors_count','air_area_count',
 'air_genre_area_count']]
feature_names = col
...
X_valid['visitor_pred_xgb02'] = np.expm1(pred_cv)

sub_file = inDir +'/submissions/Prav_xgb02_validation_preds.csv'
X_valid[['air_store_id','visit_date','visitor_pred_xgb02']].to_csv(sub_file, index=False)
