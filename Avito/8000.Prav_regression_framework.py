...
def train_xgboost_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,xgbParameters, num_rounds,current_seed, verboseeval):
    ...
    xgbbuild = xgb.DMatrix(X_build[feature_names], label=X_build['deal_probability'])
    xgbval = xgb.DMatrix(X_valid[feature_names], label=X_valid['deal_probability'])
...
def fulltrain_xgboost_regression(trainingSet, testingSet,feature_names,nbags,ModelName,xgbParameters, num_rounds,current_seed,verboseeval):
    ...
    xgbtrain = xgb.DMatrix(trainingSet[feature_names], label=trainingSet['deal_probability'])
...
def train_regression(RegressionModelName,trainingSet, testingSet,feature_names,i,nbags,ModelName,current_seed):
    if RegressionModelName in ["rf","ada","gbdt","et","lr","lsvc","knn","rgf"]:    
        X_build = trainingSet[trainingSet['CVindices'] != i]
        X_valid   = trainingSet[trainingSet['CVindices'] == i]      
        ...
        for j in range(1,nbags+1):
            ...
            if RegressionModelName == "rf":
                RegressionModel = RandomForestRegressor(n_estimators=500, max_depth=10, n_jobs=20, random_state=bag_seed, max_features="auto",verbose=0)
            ...
            if RegressionModelName == "lr":
                RegressionModel = LogisticRegression(n_jobs=20,class_weight='balanced',C=1,random_state=bag_seed)
            if RegressionModelName == "knn":
                RegressionModel = KNeighborsRegressor(n_neighbors=2,n_jobs=20)
            if RegressionModelName == "rgf":
                RegressionModel = RGFRegressor(max_leaf=1000,algorithm="RGF",loss="LS",l2=0.01,sl2=0.01,normalize=False,min_samples_leaf=10,n_iter=None,opt_interval=100,learning_rate=0.05,verbose=True)
                
            RegressionModel.fit(X_build[feature_names],X_build['target'])
            if RegressionModelName == "lr":
                bag_cv = RegressionModel.predict_proba(X_valid[feature_names])[:,1]
            bag_cv  = RegressionModel.predict(X_valid[feature_names]) 
            
            if RegressionModelName == "lr":
                pred_test += RegressionModel.predict_proba(testingSet[feature_names])[:,1]
...
def fulltrain_regression(RegressionModelName,trainingSet, testingSet,feature_names,nbags,ModelName,current_seed):
    if RegressionModelName in ["rf","ada","gbdt","et","lr","lsvc","knn","rgf"]:    
        ...
        for j in range(1,nbags+1):
            ...
            bag_seed = current_seed + j
            if RegressionModelName == "rf":
                RegressionModel = RandomForestRegressor(n_estimators=600, max_depth=10, n_jobs=20, random_state=bag_seed, max_features="auto",verbose=0)
            if RegressionModelName == "ada":
            ...
            if RegressionModelName == "lr":
                predfull_test += RegressionModel.predict_proba(testingSet[feature_names])[:,1]
...
def train_xgboost_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,xgbParameters, num_rounds,current_seed, verboseeval):
    ...
    build_feature_path = inDir + "\\input\\Prav.build.fold"+str(i)+".csv"
    valid_feature_path = inDir + "\\input\\Prav.val.fold"+str(i)+".csv"
    test_feature_path = inDir + "\\input\\Prav.test.fold"+str(i)+".csv"
    
    build_features = pd.read_csv(build_feature_path)
    valid_features = pd.read_csv(valid_feature_path)
    test_features = pd.read_csv(test_feature_path)
    
    X_build = pd.merge(X_build,build_features, how='left', on = 'item_id')
    X_valid = pd.merge(X_valid,valid_features, how='left', on = 'item_id')
    testingSet1 = pd.merge(testingSet,test_features, how='left', on = 'item_id')
    
    feature_names = cols = [col for col in X_build.columns if col not in ['item_id','deal_probability','CVindices']]
    ...
    xgtest = xgb.DMatrix(testingSet1[feature_names])
                 
    pred_cv = np.zeros(X_valid.shape[0])
    pred_test = np.zeros(testingSet1.shape[0])
