...
def gru_attention_text_model():      
    ...
    model5.add(Dense(80))
    model5.add(PReLU()) # act   
    
    
    model5.add(Dropout(rate_drop_dense))
    model5.add(BatchNormalization())
...
optim_type = 'Adam'
learning_rate = 1e-3
MODEL_WEIGHTS_FILE = 'Prav_nn12_weights.h5'
fullepochs = 9
...
#    train_data_sqnm_val  = train_data_sqnm[valindex]
    ...
    for j in range(1,nbags+1):
        ...
        model = gru_attention_text_model()
...
#        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=patience, min_lr=1e-9, epsilon = 0.00001, verbose=VERBOSEFLAG)
        ...
        model.fit([data_1_train], labels_train, 
        ...
        bag_cv   = model.predict([data_1_val], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
        pred_cv += model.predict([data_1_val], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
        
        pred_test += model.predict([test_data_1], batch_size=batch_size, verbose=VERBOSEFLAG)[:,0]
    ...
    sub_valfile = inDir+'/submissions/Prav.nn12.fold' + str(i) + '.csv'    
    ...
    sub_file = inDir+'/submissions/Prav.nn12.fold' + str(i) + '-test' + '.csv'
...
#        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=patience, min_lr=1e-9, epsilon = 0.00001, verbose=VERBOSEFLAG)
    ...
    pred_test = pred_test[['item_id','deal_probability']]
    pred_test['deal_probability'] = pred_test['deal_probability'].clip(0.0, 1.0)
    sub_file = inDir+'/submissions/Prav.nn12.full' + '.csv'
    pred_test.to_csv(sub_file, index=False)
