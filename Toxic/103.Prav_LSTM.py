...
# saving
with open(inDir+'/tokenizer.pickle', 'wb') as handle:
...
MODEL_WEIGHTS_FILE = inDir + '/103_Prav_LSTM.h5'
...
#    print('Fold ', i, '- logloss:', fold_score)
    ...
    del pred_cv
    del pred_test
    model.save(inDir+'/NLPModel.h5')
    model = load_model(inDir+'/NLPModel.h5')
...
inDir = 'C:/Users/SriPrav/Documents/R/42Toxic'
#test  = pd.read_csv(inDir+'/input/test.csv')
test1 = pd.read_csv(inDir+'/input/test1.csv')
list_classes = ["toxic", "severe_toxic", "obscene", "threat", "insult", "identity_hate"]   
list_sentences_test = test1["comment_text"].fillna("CVxTz").values
...
# loading
with open(inDir+'/tokenizer.pickle', 'rb') as handle:
...
pred_test = pd.DataFrame(pred_test)
pred_test.columns = list_classes
pred_test["id"] = test1.id.values
pred_test = pred_test[["id",'toxic', 'severe_toxic', 'obscene', 'threat', 'insult', 'identity_hate']]
sub_file = inDir+'/NLPResults' + '.csv'
pred_test.to_csv(sub_file, index=False)
