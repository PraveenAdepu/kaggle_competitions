...
# min(all_data_full_valid$Patient_ID)
# max(all_data_full_valid$Patient_ID)
# 
# ####################################################################################################################################
# set.seed(20)
...
# factor.features     <- grep("ChronicIllness", names(all_data_full), value=TRUE)
# # sapply(all_data[,factor.features], class)
# head(all_data_full[,factor.features])
# unique(all_data_full[,factor.features])
# for (f in factor.features) {
#   #if (class(train_test[[f]])=="character") {
#   cat("VARIABLE : ",f,"\n")
#   levels <- unique(all_data_full[[f]])
#   all_data_full[[f]] <- as.integer(factor(all_data_full[[f]], levels=levels))
#   #}
# }
...
all_data$gender <- as.integer(as.factor(all_data$gender))
...
# 1  - bag Processing
# X_build training Processing
# [1]	val-auc:0.834679	train-auc:0.828936 
# [51]	val-auc:0.977974	train-auc:0.985263 
# [101]	val-auc:0.980270	train-auc:0.988132 
# [151]	val-auc:0.980236	train-auc:0.989290 
# [155]	val-auc:0.980238	train-auc:0.989341 
# X_val prediction Processing

write.csv(val_predictions,  './submissions/Prav_validation2015_02.csv', row.names=FALSE, quote = FALSE)
##################################################################################################################
score(validation$DiabetesDispense, pred_cv, metric)

validation$pred_cv <- pred_cv

KnownPatientsValidation <- subset(validation,  Patient_ID < 279201)
score(KnownPatientsValidation$DiabetesDispense, KnownPatientsValidation$pred_cv, metric)
#Area under the curve: 0.9805

UnKnownPatientsValidation <- subset(validation,  Patient_ID >= 279201)
score(UnKnownPatientsValidation$DiabetesDispense, UnKnownPatientsValidation$pred_cv, metric)
# Area under the curve: 0.9804
#################################################################################################################
all_data_full <- as.data.frame(all_data_full)
all_DiabetesDispense <- unique(all_data_full[,c("Patient_ID","ChronicIllness")])

all_DiabetesDispense <- subset(all_DiabetesDispense, ChronicIllness == "Diabetes")
all_DiabetesDispense$DiabetesDispense <- 1
all_DiabetesDispense$ChronicIllness   <- NULL

all_DiabetesDispense <- unique(all_DiabetesDispense)

all_patients2016 <- subset(all_DiabetesDispense,  Patient_ID < 279201)


all_patientsFor2016 <- subset(Patients, Patient_ID < 279201 )
all_patientsFor2016 <- left_join(all_patientsFor2016,all_patients2016 , by = "Patient_ID")

all_patientsFor2016[is.na(all_patientsFor2016)] <- 0
all_patient2016Pred <- left_join(all_patientsFor2016, KnownPatientsValidation[,c("Patient_ID","pred_cv")] , by = "Patient_ID")


all_patient2016Pred$pred_cv[is.na(all_patient2016Pred$pred_cv)] <- -100

all_patient2016Pred <- arrange(all_patient2016Pred,pred_cv)
all_patient2016Pred$pred_cv_rank <-  rank(all_patient2016Pred$pred_cv)
all_patient2016Pred$pred_cv_rank <- all_patient2016Pred$pred_cv_rank / max(all_patient2016Pred$pred_cv_rank)

score(all_patient2016Pred$DiabetesDispense, all_patient2016Pred$pred_cv_rank, metric)
#Area under the curve: 0.9299

#################################################################################################################

BenchMark <- read.csv("./submissions/Prav_benchmark_01.csv")
val_sub02 <- read.csv("./submissions/Prav_validation2015_02.csv")

MergeSub <- left_join(BenchMark, val_predictions, by ="Patient_ID")

tail(MergeSub,100)

MergeSub$Diabetes.y[is.na(MergeSub$Diabetes.y)] <- -100
...
write.csv(Final_sub,  './submissions/Prav_xgb02.csv', row.names=FALSE, quote = FALSE)
#################################################################################################################
