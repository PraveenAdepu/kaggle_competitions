...
def train_xgboost_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,xgbParameters, num_rounds,current_seed, verboseeval):
    ...
    xgbbuild = xgb.DMatrix(X_build[feature_names], label=X_build['y'])
    xgbval = xgb.DMatrix(X_valid[feature_names], label=X_valid['y'])
    ...
    for j in range(1,nbags+1):
        ...
        pred_cv += bag_cv
        bag_score = r2_score(X_valid['y'], bag_cv)
        print('bag ', j, '- r2_score :', bag_score)
    pred_cv /= nbags
    pred_test/= nbags
    fold_score = r2_score(X_valid['y'], pred_cv)
    ...
    pred_cv[["ID","y"]].to_csv(sub_valfile, index=False)
    ...
    pred_test[["ID","y"]].to_csv(sub_file, index=False)
    del pred_cv
    del pred_test
...
def fulltrain_xgboost_regression(trainingSet, testingSet,feature_names,nbags,ModelName,xgbParameters, num_rounds,current_seed,verboseeval):
    fullnum_rounds = int(num_rounds * 1.1)
    xgbtrain = xgb.DMatrix(trainingSet[feature_names], label=trainingSet['y'])
...
def train_lgbm_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,lgbmParameters, lgbm_num_rounds,current_seed,verboseeval):
    ...
    lgbmbuild = lightgbm.Dataset(X_build[feature_names], X_build['y'])
    lgbmval   = lightgbm.Dataset(X_valid[feature_names], X_valid['y'])
    ...
    for j in range(1,nbags+1):
        ...
        model = lightgbm.train(lgbmParameters, lgbmbuild,lgbm_num_rounds,feval=lgbm_r2_score,valid_sets=lgbmval,verbose_eval = verboseeval
...
def fulltrain_lgbm_regression(trainingSet, testingSet,feature_names,nbags,ModelName,lgbmParameters, lgbm_num_rounds,current_seed,verboseeval):
    ...
    lgbmtrain = lightgbm.Dataset(trainingSet[feature_names], trainingSet['y'])

    predfull_test = np.zeros(testingSet.shape[0]) 
    for j in range(1,nbags+1):
        print('bag ', j , ' Processing')
        lgbmParameters['seed'] =  current_seed + j           
        fullmodel = lightgbm.train(lgbmParameters, lgbmtrain,full_lgbm_num_rounds,feval=lgbm_r2_score,verbose_eval = verboseeval
                                  #,early_stopping_rounds=early_stopping_rounds
                                  )
    ...
    if RegressionModelName in ["rf","ada","gbdt","et","lr","lsvc","knn"]:    
        ...
        for j in range(1,nbags+1):
            ...
            if RegressionModelName == "rf":
                RegressionModel = RandomForestRegressor(n_estimators=610, max_depth=4, n_jobs=10, random_state=bag_seed, max_features="auto",verbose=0)
            ...
            if RegressionModelName == "et":
                RegressionModel = ExtraTreesRegressor(n_estimators=610, max_depth=4, max_features="auto", n_jobs=10, random_state=bag_seed,verbose=0)
            ...
            RegressionModel.fit(X_build[feature_names],X_build['y'])
...
def fulltrain_regression(RegressionModelName,trainingSet, testingSet,feature_names,nbags,ModelName,current_seed):
    if RegressionModelName in ["rf","ada","gbdt","et","lr","lsvc","knn"]:    
        ...
        for j in range(1,nbags+1):
            ...
            bag_seed = current_seed + j
            if RegressionModelName == "rf":
                RegressionModel = RandomForestRegressor(n_estimators=670, max_depth=4, n_jobs=10, random_state=bag_seed, max_features="auto",verbose=0)
            if RegressionModelName == "ada":
            ...
            if RegressionModelName == "et":
                RegressionModel = ExtraTreesRegressor(n_estimators=670, max_depth=4, max_features="auto", n_jobs=10, random_state=bag_seed,verbose=0)
            if RegressionModelName == "lr":
                RegressionModel = LinearRegression(n_jobs=10) 
            if RegressionModelName == "knn":
                RegressionModel = KNeighborsRegressor(n_neighbors=2,n_jobs=20)
                
            RegressionModel.fit(trainingSet[feature_names],trainingSet['y'])    
