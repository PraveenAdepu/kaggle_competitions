...
pred_cv_forecasts <- cbind(X_valid[,c("fecha_dato","ncodpers","target","targetLabel","flag_new")],pred_cv_matrix)
...
pred_cv_forecasts_long <- melt(pred_cv_forecasts, id.vars=c("fecha_dato", "ncodpers", "target", "targetLabel","flag_new"))
...
pred_cv_forecasts_long_top7 <- pred_cv_forecasts_long[, head(.SD, 7), by = c("fecha_dato","ncodpers","targetLabel","flag_new")]
rm(pred_cv_forecasts_long);
gc()

# in submission format
pred_cv_submission <- pred_cv_forecasts_long_top7[,.(added_products=paste0(variable,collapse=" ")), keyby=c("ncodpers","targetLabel","flag_new") ]; gc() #Build submission
rm(pred_cv_forecasts_long_top7);
gc()
head(pred_cv_submission)

# MAP@7 - validation score
validation_mapk7_score <- mapk(7,pred_cv_submission$targetLabel,strsplit(pred_cv_submission$added_products," "))
cat("validation mapk7 score - ", validation_mapk7_score)

pred_cv_submission_newProducts <- pred_cv_submission %>% filter(flag_new == 1)
head(pred_cv_submission_newProducts)

# MAP@7 - validation score
validation_mapk7_score_newProducts <- mapk(7,pred_cv_submission_newProducts$targetLabel,strsplit(pred_cv_submission_newProducts$added_products," "))
cat("validation mapk7 score newProducts - ", validation_mapk7_score_newProducts)

# #########################################################################################################


write.csv(pred_cv_forecasts,  paste0(root_directory,'/input/Prav.xgb.Model05-cv.csv'), row.names=FALSE, quote = FALSE)
write.csv(pred_test_forecasts, paste0(root_directory,'/input/Prav.xgb.Model05-test.csv'), row.names=FALSE, quote = FALSE)
write.csv(impMatrix, paste0(root_directory,'/input/Prav.xgb.Model05-VariableImportance.csv'), row.names=FALSE, quote = FALSE)
