...
trainingSet['CVindices'] = np.where(trainingSet['date']>='2017-07-26', 1, 2)
...
list(set(trainingSet.item_nbr.values) - set(testingSet.item_nbr.values))

set(testingSet.item_nbr.unique()) - set(trainingSet.item_nbr.unique())

testingSet[(~testingSet.item_nbr.isin(trainingSet.item_nbr))].item_nbr.nunique()

feature_names = [c for c in trainingSet.columns if c not in ['date','unit_sales','CVindices']]

trainingSet[['date', 'store_nbr', 'item_nbr']].groupby(['date','store_nbr']).agg(['count'])
...
for i in range(1, folds+1):
        train_lgbm_regression(trainingSet, trainingSet,feature_names,i,nbags,lgbmModelName,lgbm_params, lgbm_num_round,current_seed,verboseeval)
