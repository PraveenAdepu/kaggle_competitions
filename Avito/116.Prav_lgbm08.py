...
trainingSet = df[0:traindex.shape[0]] #ready_df[0:traindex.shape[0]]
testingSet = df[traindex.shape[0]:] #ready_df[traindex.shape[0]:]
...
def train_lgbm_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,lgbmParameters, lgbm_num_rounds,current_seed,verboseeval):
    ...
    lgbmbuild = lightgbm.Dataset(X_build[feature_names], X_build['deal_probability'], feature_name=feature_names,
                categorical_feature = categorical,free_raw_data=False)
    lgbmval   = lightgbm.Dataset(X_valid[feature_names], X_valid['deal_probability'], feature_name=feature_names,
                categorical_feature = categorical,free_raw_data=False)
...
#    lgbmbuild = lightgbm.Dataset(X_build, y_build, feature_name=feature_names,
#                categorical_feature = categorical
#                )
#
#    lgbmval   = lightgbm.Dataset(X_valid, y_valid, feature_name=feature_names,
#                categorical_feature = categorical)
...
def fulltrain_lgbm_regression(trainingSet, testingSet,feature_names,nbags,ModelName,lgbmParameters, lgbm_num_rounds,current_seed,verboseeval):
    ...
    lgbmtrain = lightgbm.Dataset(trainingSet[feature_names], trainingSet['deal_probability'], feature_name=feature_names,
                categorical_feature = categorical,free_raw_data=False)
