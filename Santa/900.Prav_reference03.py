...
def recalc_hapiness(happiness, best_gifts, gift):
    ...
    for h in happiness:
        ...
        if c <= 45000 and happiness[h] < 0.1:
            happiness[h] = 0.1
...
def solve():
    ...
    gift_init = pd.read_csv(inDir + '/input/gift_goodkids_v2.csv', header=None).as_matrix()[:, 1:]
    gift = gift_init.copy()
    ...
    out = open(inDir+'/submissions/Prav_sub_{}.csv'.format(score), 'w')
