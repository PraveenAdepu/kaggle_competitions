...
def train_xgboost_regression_usingIndex(train, test,X_train,y_train,X_test,i,nbags,ModelName,xgbParameters, num_rounds,current_seed, verboseeval):
    
#    X_build = trainingSet[trainingSet['CVindices'] != i]
#    X_valid   = trainingSet[trainingSet['CVindices'] == i]
    
    trainindex = train[train['CVindices'] != i].index.tolist()
    valindex   = train[train['CVindices'] == i].index.tolist()
    
    X_val_df = train.iloc[valindex,:]
    ...
    for j in range(1,nbags+1):
        ...
        pred_cv += bag_cv
        bag_score = log_loss(y_valid, bag_cv)
        print('bag ', j, '- log_loss :', bag_score)
    pred_cv /= nbags
    pred_test/= nbags
    fold_score = log_loss(y_valid, pred_cv)
    print('Fold ', i, '- log_loss :', fold_score)
    ...
    pred_cv[["id","is_iceberg"]].to_csv(sub_valfile, index=False)
    ...
    pred_test[["id","is_iceberg"]].to_csv(sub_file, index=False)
    del pred_cv
    del pred_test
...
def train_lgbm_regression(trainingSet, testingSet,feature_names,i,nbags,ModelName,lgbmParameters, lgbm_num_rounds,current_seed,verboseeval):
    ...
    for j in range(1,nbags+1):
        ...
        model = lightgbm.train(lgbmParameters, lgbmbuild,lgbm_num_rounds,feval=gini_lgb,valid_sets=lgbmval,verbose_eval = verboseeval
...
def fulltrain_lgbm_regression(trainingSet, testingSet,feature_names,nbags,ModelName,lgbmParameters, lgbm_num_rounds,current_seed,verboseeval):
    ...
    for j in range(1,nbags+1):
        print('bag ', j , ' Processing')
        lgbmParameters['seed'] =  current_seed + j           
        fullmodel = lightgbm.train(lgbmParameters, lgbmtrain,full_lgbm_num_rounds,feval=gini_lgb,verbose_eval = verboseeval
                                  #,early_stopping_rounds=early_stopping_rounds
                                  )
