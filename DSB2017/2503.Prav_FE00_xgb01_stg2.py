...
def train_xgboost(i):
    ...
    clf.fit(trn_x, trn_y, eval_set=[(val_x, val_y)], verbose=True, eval_metric='logloss', early_stopping_rounds=25)
    pred_valid = np.clip(clf.predict(val_x),0.001,1)
    pred_cv['cancer'] = pred_valid
    sub_valfile = 'C:/Users/SriPrav/Documents/R/19DSB2017/submissions/Prav.FE00.bl02.fold_stg2' + str(i) + '.csv'
    pred_cv.to_csv(sub_valfile, index=False)
    
    pred_test = np.clip(clf.predict(test_x),0.001,1)
    test_df['cancer'] = pred_test
    
    sub_file = 'C:/Users/SriPrav/Documents/R/19DSB2017/submissions/Prav.FE00.bl02.fold_stg2' + str(i) + '-test' + '.csv'
...
def fulltrain_xgboost(bags):
    ...
    sub_file = 'C:/Users/SriPrav/Documents/R/19DSB2017/submissions/Prav.FE00.bl02_stg2.full' + str(i) + '-bags' + '.csv'
    test_df.to_csv(sub_file, index=False)
